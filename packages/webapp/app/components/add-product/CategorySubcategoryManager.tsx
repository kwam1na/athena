...
function CategoryManager() {
  ...
  const update = async () => {
    if (!categoryIdToRename || !updatedName || !store) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["subcategories", store?.id],
      });
    },
    onError: (e) => {
      toast("Something went wrong", {
