/**
 * SHA-256 content digests for harness evidence (browser-safe, dependency-free).
 *
 ...
 * transaction-bound helper could not await without restructuring.
 */
...
/**
 * `sha256:<hex>` over the canonical JSON encoding of a value (sorted keys,
 * `undefined` dropped), so two equal JSON values always share one digest.
 */
