...
| definition modules | `definitions.ts`, `readDefinitions.ts`, `domains/**` reference no environment reader (`process`, `Deno`, `globalThis`, `self`, `window`, `import.meta`) — the checker evaluates them in its own process and proves identity against what it loaded, so an environment-dependent field would be verified under one environment and enforced under another (round 7) | `definition-module-reads-environment` |
