...
11. **The router class is resolved, not spelled** (round 10). Every round-9
   ...
   read in one, the right side of an assignment), or the argument of a
   TESTED predicate (`if (isX(error))`, `!isX(error)`) — `Object.assign(error,
   …)`, `error.x = …`, `[error] = …`, `({ error } = …)`, `const e = error`,
   `decorate(error)`, `error.setStatus(…)` are violations.
