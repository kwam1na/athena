...
import type { OfflineAuthorityReceiptVerification } from "../offlineAuthorityReceipt";
import type {
...
describe("createLocalSyncIngestionService", () => {
  it("projects service-principal events whose offline authority receipt verifies", async () => {
    const repository = createFakeSyncRepository();
    const offlineAuthorityVerifier = vi.fn(
      async (): Promise<OfflineAuthorityReceiptVerification> => ({
        disposition: "valid",
        payload: buildOfflineAuthorityPayload(),
      }),
    );
    ...
    const result = await service.ingestBatch(
      buildBatch({ enforceOfflineAuthorityReceipt: true, events: [event] }),
    );

    expect(result).toMatchObject({
      kind: "ok",
      data: {
        accepted: [{ localEventId: event.localEventId, status: "projected" }],
        syncCursor: { acceptedThroughSequence: 1 },
      },
    });
    expect(offlineAuthorityVerifier).toHaveBeenCalledWith({
  ...
  it("durably holds missing offline authority as stable review evidence", async () => {
    const repository = createFakeSyncRepository();
    const service = createLocalSyncIngestionService({
      repository,
      projectionRepository: repository,
      now: () => 100,
      offlineAuthorityVerifier: async () => ({
        ...
        conflicts: [
          {
            ...
            status: "needs_review",
            details: { reason: "missing_receipt" },
          },
        ],
        syncCursor: { acceptedThroughSequence: 1 },
      },
  ...
  it("persists verified post-expiry evidence without storing the receipt", async () => {
    const repository = createFakeSyncRepository();
    const service = createLocalSyncIngestionService({
      repository,
      projectionRepository: repository,
      ...
      buildBatch({
        ...
        events: [
          buildSaleCompletedEvent({
            sequence: 1,
            offlineAuthorityReceipt: "private-signed-envelope",
          }),
        ],
      }),
    );
    ...
    expect(JSON.stringify(repository.conflicts)).not.toContain(
  ...
  it.each([
    {
      name: "forged or cross-scope authority",
      verification: {
        disposition: "rejected",
        reason: "scope_mismatch",
      } satisfies OfflineAuthorityReceiptVerification,
      code: "authorization_failed",
      retryable: undefined,
    },
    {
      name: "authority verification infrastructure failure",
      verification: {
  ...
  ])("rejects $name before any cursor-advancing write", async ({ verification, code, retryable }) => {
    const repository = createFakeSyncRepository();
    const service = createLocalSyncIngestionService({
      repository,
      projectionRepository: repository,
      now: () => 100,
      offlineAuthorityVerifier: async () => verification,
    });
  ...
  it("does not report or advance a plain rejected event on the receipt-enforced lane", async () => {
    const repository = createFakeSyncRepository();
    const service = createLocalSyncIngestionService({
      repository,
      projectionRepository: repository,
    ...
    const event = buildPendingCheckoutItemDefinedEvent({
      ...
      offlineAuthorityReceipt: "receipt-1",
      payload: {
        ...buildPendingCheckoutItemDefinedEvent({ sequence: 1 }).payload,
        name: "",
      },
    });
    const batch = buildBatch({
    ...
    for (const result of [first, retry]) {
      expect(result).toMatchObject({
        ...
        data: {
          accepted: [],
          conflicts: [],
          syncCursor: { acceptedThroughSequence: 0 },
        },
      });
    }
  ...
  it("advances a receipt-enforced money reject only as stable review evidence", async () => {
    const repository = createFakeSyncRepository();
    const service = createLocalSyncIngestionService({
      repository,
      projectionRepository: repository,
    ...
    const original = buildSaleCompletedEvent({ sequence: 1 });
    const event = buildSaleCompletedEvent({
      sequence: 1,
      offlineAuthorityReceipt: "receipt-1",
      payload: {
        ...original.payload,
        totals: { subtotal: 25, tax: 0, total: 24 },
      },
    });
    const batch = buildBatch({
      enforceOfflineAuthorityReceipt: true,
    ...
    expect(first).toMatchObject({
      ...
      data: {
        accepted: [{ status: "conflicted" }],
        conflicts: [{ conflictType: "server_rejected", status: "needs_review" }],
        syncCursor: { acceptedThroughSequence: 1 },
      },
    });
    expect(retry).toEqual(first);
...
function buildOfflineAuthorityPayload() {
  return {
    ...
    storeId: "store-1",
    terminalId: "terminal-1",
    terminalLifecycleRevision: 5,
    terminalProofRevision: 6,
    version: 1 as const,
  };
}
