...
/**
 * Keys whose values are generated by the server from a compile-time catalog
 * and never travel through a browser: capability ids and operation ids. They
 * match a strict identifier shape and skip the free-text sensitivity scan,
 * because that scan is tuned for visitor-authored text and a future catalog id
 * (`payment.collect`, say) would trip it on a word boundary.
 *
 * `surfaceKey` deliberately does NOT belong here. It is an allowed payload key
 * on the client-reportable events, so a crafted browser could put anything
 * identifier-shaped in it. It stays on the normal validation path, scan
 * included.
 */
const CLOSED_IDENTIFIER_KEYS = new Set(["capability", "operationId"]);
