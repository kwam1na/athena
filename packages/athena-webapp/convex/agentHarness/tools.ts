...
export const executeProgramTool: AgentToolDefinition<AgentExecuteProgramArgs, unknown> = {
  ...
  description:
    ...
    "SPEED: calls run up to 4 at a time — `Promise.all` over a batch is much faster than awaiting one by one, and excess calls queue rather than fail. Repeating an identical read inside one program is served from cache but still counts toward the program's call budget — reuse the variable instead.\n" +
