...
describe("ensure gates", () => {
  ...
  it("rejects malformed dates and spans beyond the 92-day movement ceiling", async () => {
    const t = convexTest(schema, modules);
    const { storeId } = await seedStore(t);
    allow(storeId);
    await expect(
    ...
    await expect(
      ensure(t, { storeId, startDate: "2026-01-01", endDate: "2026-04-03" }),
    ).rejects.toThrow(/maximum is 92 days/);
  });
  ...
  it("admits a full 92-day range of empty days within the bounded ensure read", async () => {
    const t = convexTest(schema, modules);
    const { storeId } = await seedStore(t);
    allow(storeId);
    // 2026-01-01 .. 2026-04-02 inclusive is exactly 92 days, all empty.
    const result = await ensure(t, {
      storeId,
