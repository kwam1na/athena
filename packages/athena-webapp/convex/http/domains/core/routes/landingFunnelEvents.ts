...
const LANDING_FUNNEL_MAX_BODY_BYTES = 1_024;
...
// Anonymous by design and anonymous in fact: four coarse milestones, no
// identity of any kind read from the request.
landingFunnelEventRoutes.use(
  "*",
  boundRequestBody(() => LANDING_FUNNEL_MAX_BODY_BYTES),
);
landingFunnelEventRoutes.post(
  "/",
  admitHttpRoute(
    ...
    async (c, { ingress }) => {
      ...
      if (contentLength !== undefined && (!Number.isSafeInteger(contentLength) || contentLength < 0 || contentLength > LANDING_FUNNEL_MAX_BODY_BYTES)) return c.json({ error: { code: "request_rejected" } }, 413);
