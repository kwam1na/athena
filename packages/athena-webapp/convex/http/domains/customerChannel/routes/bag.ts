...
import { getBagRouteReadDefinition } from "../../../../operationAdmission/domains/httpCustomer_readDefinitions";
import {
  admittedCustomerId,
...
bagRoutes.post(
  ...
  admitHttpRoute(updateBagOwnerRouteOperationDefinition, async (c, admitted) => {
    ...
    try {
      ...
      // Ownership refusal → 403, malformed guest id → 400, anything else is a
      // real fault and propagates. See `guestMergeErrorResponse`.
