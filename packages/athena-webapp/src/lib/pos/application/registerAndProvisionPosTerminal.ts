...
export async function reactivateAndProvisionPosTerminal(input: {
  expectedStoreId: Id<"store">;
  expectedTerminalId: Id<"posTerminal">;
  fingerprintHash: string;
  reconnectIntentToken: string;
  reactivateTerminalMutation: (args: {
    browserFingerprintHash: string;
    nextSyncSecretHash: string;
    reconnectIntentToken: string;
  }) => Promise<
    | { kind: "ok"; data: ProvisionedTerminalRecord }
    | { kind: "user_error"; error: { message: string } }
  ...
  const store = input.storeFactory();
  const storedSeed = await store.readProvisionedTerminalSeed();
  if (!storedSeed.ok) {
    throw new Error(storedSeed.error.message);
  }
  if (!storedSeed.value) {
    throw new Error("A provisioned terminal seed is required for reactivation.");
  }

  const seed = storedSeed.value;
  if (seed.terminalId !== input.fingerprintHash) {
    throw new Error(
      "POS reactivation must preserve the same terminal fingerprint.",
    );
  }
  if (
    seed.storeId !== input.expectedStoreId ||
    seed.cloudTerminalId !== input.expectedTerminalId
  ) {
    throw new Error(
      "POS reactivation must match the stored terminal and store lineage.",
    );
  }

  const nextSyncSecretHash = await createTerminalSyncSecretToken();
  const result = await input.reactivateTerminalMutation({
    browserFingerprintHash: input.fingerprintHash,
    nextSyncSecretHash,
    reconnectIntentToken: input.reconnectIntentToken,
  });
  if (result.kind === "user_error") return result;

  if (
    result.data._id !== seed.cloudTerminalId ||
    result.data.storeId !== seed.storeId ||
    result.data.fingerprintHash !== seed.terminalId ||
    result.data.syncSecretHash !== nextSyncSecretHash
  ) {
    throw new Error(
      "POS reactivation must return the same terminal record and store lineage.",
    );
  }

  const seedWithoutReceipt = { ...seed };
  delete seedWithoutReceipt.offlineAuthorityReceipt;
  const nextSeed = {
    ...seedWithoutReceipt,
    syncSecretHash: nextSyncSecretHash,
    provisionedAt: input.now?.() ?? Date.now(),
  };
  const seedWrite = store.writeProvisionedTerminalSeedAndClearTerminalIntegrity
    ? await store.writeProvisionedTerminalSeedAndClearTerminalIntegrity({
    ...
    : await store.writeProvisionedTerminalSeed(nextSeed);
  if (!seedWrite.ok) {
    throw new Error(seedWrite.error.message);
  }
  if (
