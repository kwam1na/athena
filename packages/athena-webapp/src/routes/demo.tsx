    ...
    <main className="flex min-h-svh items-center justify-center bg-app-canvas px-layout-md py-layout-xl">
      <section aria-labelledby="demo-entry-title" className="w-full max-w-lg border-y border-border bg-background py-layout-xl text-center sm:px-layout-lg">
        ...
        <h1 id="demo-entry-title" className="mt-layout-md font-display text-4xl font-light text-foreground">
          {failed ? "The shared demo is not available right now." : "Opening the shared demo store…"}
        </h1>
        <p className="mx-auto mt-layout-md max-w-md leading-7 text-muted-foreground">
          {failed ? "Athena could not open the synthetic store. Try again in a moment." : "No account or credentials are needed. This may take a few seconds."}
        </p>
        {failed ? <Button type="button" size="lg" className="mt-layout-lg" onClick={() => { started.current = true; void enter(); }}>Try again</Button> : null}
        {!failed ? <span className="sr-only" role="status">Signing in to the shared demo</span> : null}
      </section>
    </main>
  );
}
