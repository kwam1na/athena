...
import { Activity, Eye, MonitorCheck, ShoppingBag, Users } from "lucide-react";
...
export default function AnalyticsView() {
  ...
  const summary = useQuery(
    ...
    activeStore?._id ? { storeId: activeStore._id, currentTimeMs } : "skip",
  );
    ...
    return (
      <div className="py-layout-xl">
        <h2 className="font-display text-2xl text-foreground">
          Storefront activity unavailable
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Live storefront activity could not be loaded. Try again shortly.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-layout-xl py-layout-lg">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">
          Independent live timeframe
        </p>
        <h2 className="mt-2 font-display text-2xl">Storefront activity</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Live engagement activity uses its own timeframe and does not change
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="max-w-2xl space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Start with current movement, then inspect shoppers, products,
                  and journey health when the signal changes.
                </p>
              </div>
