...
function ProductViewContent() {
  ...
  const modifyProduct = async () => {
    ...
    try {
      ...
      if (res.some((result) => result === false)) {
        return;
      }
  ...
  const createVariantSku = async (
    ...
    const moneyPayload = buildVariantSkuMoneyPayload(
      variant,
    ...
    return await createSku({
      ...
      barcodeAutoGenerated: variant.barcodeAutoGenerated,
      price: moneyPayload.price,
      netPrice: moneyPayload.netPrice,
  ...
  const updateVariantSku = async (skuId: string, variant: ProductVariant) => {
    if (variant.images && variant.images.length > 0) {
      try {
    ...
    const persistedSku = activeProduct?.skus.find(
      (sku) => String(sku._id) === skuId,
    );
    const correctedInventoryCount = variant.stock ?? 0;
    const correctedQuantityAvailable = variant.quantityAvailable ?? 0;
    const correctedUnitCostMinor = moneyPayload.unitCost ?? null;
    if (
      persistedSku &&
      (persistedSku.inventoryCount !== correctedInventoryCount ||
        persistedSku.quantityAvailable !== correctedQuantityAvailable ||
        (persistedSku.unitCost ?? null) !== correctedUnitCostMinor)
    ) {
      await correctSkuValuation({
        inventoryCount: correctedInventoryCount,
        productSkuId: skuId as Id<"productSku">,
        quantityAvailable: correctedQuantityAvailable,
        reason: "Product editor stock and unit cost correction",
        requestKey: `product-editor:${skuId}:${crypto.randomUUID()}`,
        storeId: activeStore!._id,
        unitCostMinor: correctedUnitCostMinor,
      });
    }
