...
- Order history, rewards, or online-order query changes: run `bun run --filter '@athena/storefront-webapp' test` and `bunx tsc --noEmit -p packages/storefront-webapp/tsconfig.json`. There is no storefront-only focused regression for Athena-managed returns, exchanges, or customer-history payload changes yet, so pair this with the omnichannel regression set documented in [packages/athena-webapp/docs/agent/testing.md](../../../athena-webapp/docs/agent/testing.md) when the shared API contract changes.
...
- Storefront context-event telemetry changes: target [src/lib/storefrontContextEvents.test.ts](../../src/lib/storefrontContextEvents.test.ts), [src/api/trackingEvents.test.ts](../../src/api/trackingEvents.test.ts), and the relevant journey surface test. Pair intelligence-bearing event changes with Athena compiler/readout tests for [contextBundles.test.ts](../../../athena-webapp/convex/contextTracking/contextBundles.test.ts), [insights.test.ts](../../../athena-webapp/convex/intelligence/capabilities/insights.test.ts), and readout UI debug coverage.
