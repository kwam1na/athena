...
- **Complexity-adding suggestions.** Proposals to introduce a new abstraction, helper, wrapper, configuration option, feature flag, rename, consolidation, or an extra layer of validation that the delivery contract did not ask for. Even when the idea is sound, it is not a defect in the diff. A real defect whose cleanest fix would be a refactor is still reported — with the minimal in-diff fix, not the refactor.
...
- Keep every `suggested_fix` minimal: the smallest edit inside the changed code that closes the concrete defect. Never propose a refactor, new abstraction, generalization, or additional hardening as the fix (see the scope rules' "Stay Inside The Diff, Add No Complexity").
