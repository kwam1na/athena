...
**Complexity-adding proposals are `expansion`, not fixes.** A proposal to introduce a new abstraction, helper, wrapper, configuration option, feature flag, rename, consolidation, or an extra layer of validation the contract did not ask for is never an in-delivery fix, even when the idea is sound. Report it as its own `expansion` finding with `autofix_class: gated_auto` or `manual` so it becomes deferred follow-up work; do not fold it into a defect's `suggested_fix`, and do not inflate its severity to keep it blocking. If it has no concrete rationale, it is the false-positive catalog's generic "consider adding" shape — suppress it.
...
- Keep every `suggested_fix` minimal: the smallest edit inside the changed code that closes the concrete defect. Never propose a refactor, new abstraction, generalization, or additional hardening as the fix (see the scope rules' "Stay Inside The Diff, Add No Complexity"); if such an improvement is worth doing, report it as a separate `expansion` finding so it is deferred, not dropped.
