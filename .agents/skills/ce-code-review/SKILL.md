...
- **Reviews stay inside the diff and add no complexity.** A finding that is only a proposal to add an abstraction, helper, wrapper, configuration knob, rename, consolidation, or extra validation layer the contract never asked for is never fixed in this delivery: synthesis classifies it `expansion` with `autofix_class` `gated_auto`/`manual` and routes it through the deferral partition, so it is reported in Deferred Expansion Work and resolved by a follow-up ticket — deferred, not dropped. Synthesis drops it only when it carries no concrete rationale (the false-positive catalog's generic "consider adding" shape). A concrete defect whose `suggested_fix` is such a proposal is kept, but its fix is reduced to the minimal in-diff change before it reaches the fixer, and the proposal itself is split out as a separate `expansion` finding; the fixer must not refactor, generalize, or harden beyond the finding's evidence. Count both dispositions under Coverage as complexity-deferred. See `references/diff-scope.md`, "Stay Inside The Diff, Add No Complexity".
...
**Athena evidence-bearing selection:** Before preparation, replace `.agents/review-selection.json` with the conditional reviewers selected for this candidate: an array of `{ "reviewerId": "ce-...", "reason": "concrete diff or PR applicability" }` entries. This file is the whole additional roster: `harness.config.ts` reads it and adds nothing of its own, so an ordinary Athena delivery lists the always-on reviewers above alongside the conditionals it selected. Use `[]` only when the delivery deliberately selects no reviewer beyond the product's two mandated lenses, which the installed product supplies either way. This is the caller's recorded judgment, not an automatic selector. Use existing `.agents/agents/<reviewerId>.agent.md` charters. Stage this selection file and any authored config/charter changes before running preparation, preserving unrelated staging. Do not carry a previous delivery's conditionals forward without reassessing them. Selection or charter changes after dispatch require preparation and a fresh complete review context. Ordinary report-only reviews do not change this tracked policy or emit gate evidence.
...
- For an Athena evidence-bearing review, complete and stage the selection above, run `bun run pr:athena:prepare`, then retain the exact JSON from `bun run harness:review-context` before dispatch. Fix preparation failures before spending review. Its `binding.charters` is the complete required reviewer set, including the installed product defaults and Athena's selected charters; give each reviewer that charter and the retained context. Do not edit or reconstruct the context.
- Keep the reviewer results and round history under the review run root. If fixes change the reviewed deliverable, selection, or charter inputs, prepare again and acquire the complete review on the fresh context. Preserve prior findings and rounds under the installed review workflow's delivery-wide bound. The product decides context freshness; do not substitute the retired Athena identity or manifest checks.
- After the complete prepared pass concludes, write a `review-outcome/1` document naming the retained context's `digest` as `contextDigest`, a verdict, every `binding.charters[].reviewerId` exactly once in `reviewers` as `{ "id": "...", "result": "approved|rejected|failed|timed-out" }`, and the actual `findings`. Preserve findings and their dispositions, including filed `deferredIssueId` for discharged deferrals; do not turn a missing reviewer into approval. Include actual `runHistory` and `finalPassId` when carrying multiple passes. Only a unanimously approved pass with no unresolved actionable findings can support green evidence.
- Pass that concluded document on stdin to `bun run harness:review-outcome -- --context <retained-context.json> < <review-outcome.json>`. The installed product validates the context, reviewer coverage, findings, and evidence, and returns the product manifest path. Use that returned path with `bun run harness:review-evidence -- --manifest <returned-path>`. Never hand-author the retired Athena `final-manifest.json` envelope or rename extra conditional results under always-on reviewer ids. If the host reports review cost, preserve its actual unit and total in the outcome's `cost` with an explicit `costCoverage` description; omit unreported cost rather than estimating it.
