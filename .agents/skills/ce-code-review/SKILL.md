...
- When an Athena caller explicitly requests an evidence-bearing review, run `bun run pr:athena:prepare` before the complete review pass and capture its identity with `bun run harness:review-context`. Ordinary report-only reviews do not issue gate evidence.
