...
- **Reviews stay inside the diff and add no complexity.** A finding that is only a proposal to add an abstraction, helper, wrapper, configuration knob, rename, consolidation, or extra validation layer the contract never asked for is a non-finding: synthesis drops it (counted under Coverage as complexity-suppressed), and it is neither fixed nor filed as a deferral. A concrete defect whose `suggested_fix` is such a proposal is kept, but its fix is reduced to the minimal in-diff change before it reaches the fixer; the fixer must not refactor, generalize, or harden beyond the finding's evidence. See `references/diff-scope.md`, "Stay Inside The Diff, Add No Complexity".
