...
- When an Athena caller explicitly requests an evidence-bearing review, run `bun run pr:athena:prepare` before the complete review pass and capture its identity with `bun run harness:review-context`. Preparation runs the mechanical per-package lint checks and publishes no receipt when they fail, so fix those before dispatching reviewers rather than after. Ordinary report-only reviews do not issue gate evidence.
