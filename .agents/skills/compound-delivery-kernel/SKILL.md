...
- when the delivery ran a gate that emits a run summary (Athena's `pr:athena` prints `[pr:athena]` summary lines: run status, duration vs. the last passing baseline, review-loop telemetry — iterations, finding counts by severity, deferred findings with their follow-up issue ids — and review cost in whatever unit the agent platform meters, with its delta against the last passing run), relay those lines in the handoff instead of paraphrasing them; they are the run's telemetry of record. Review cost is self-reported by the agent platform rather than independently verified, and is only comparable between runs metered the same way by the same platform, so present it as a cost trend, not as audit-grade evidence
- when the repo keeps a durable run-telemetry store, record this run into it and commit the record with the delivery, so the trend survives the worktree the run happened in. In Athena that is `bun run delivery:telemetry-record`, which promotes the gate's ledger into a tracked file under `telemetry/delivery-runs/<YYYY-MM-DD>/` — and it is enforced, not optional: the `telemetry.recorded` gate obligation and a CI check both fail a substantial delivery that carries no current record. Like the solution note, it is scaled to the delivery (nothing is owed below the repo's source-line threshold) and it goes stale when the deliverable changes after recording. Record after the gate passes, never mid-run: the gate proves a tree it already validated, and writing into that tree mid-run invalidates the proof
