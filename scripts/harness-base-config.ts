...
import { HARNESS_APP_REGISTRY } from "./harness-app-registry.ts";
...
export const ATHENA_LEGACY_CONFIG = defineHarnessConfig({
  ...
  preparationWiringPaths: ["harness.config.ts", "scripts/harness-base-config.ts", ".agents/review-selection.json", "package.json", "scripts/harness-app-registry.ts", "scripts/harness-mechanical-check.ts", "scripts/harness-review.ts", "scripts/bun-version-check.ts", "scripts/frontend-dependency-parity.ts", "scripts/pre-commit-generated-artifacts.ts"],
  preparationCommands: [
...
export type AthenaValidationMode = "legacy" | "comparison" | "full-health";
/** Pure mode selection; the caller owns whether activation has been accepted. */
export function resolveAthenaValidationMode(value: string | undefined, defaultMode: AthenaValidationMode = "legacy"): AthenaValidationMode {
  if (value === undefined || value === "") return defaultMode;
  if (value === "legacy" || value === "comparison" || value === "full-health") return value;
  throw new Error("ATHENA_VALIDATION_MODE must be legacy, comparison or full-health");
}
export default ATHENA_LEGACY_CONFIG;
