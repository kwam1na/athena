...
async function createFixtureRepo() {
  const rootDir = await mkdtemp(
    path.join(tmpdir(), "athena-harness-generate-"),
  );
    ...
    JSON.stringify(
      {
        ...
        scripts: {
          ...
          build: "vite build && tsc --noEmit",
          "lint:architecture":
            "bun ../../scripts/architecture-boundary-check.ts storefront-webapp",
        },
      },
      null,
      2,
  ...
  await write(
    "packages/athena-webapp/src/routes/login/index.tsx",
    "export {};\n",
    rootDir,
  );
  await write(
  ...
  await write(
    "packages/athena-webapp/src/lib/utils.ts",
    "export {};\n",
    rootDir,
  );
  await write(
  ...
  await write(
    "packages/athena-webapp/src/example.test.tsx",
    "export {};\n",
    rootDir,
  );
  await write(
  ...
  await write(
    "packages/storefront-webapp/src/routes/shop/index.tsx",
    "export {};\n",
    rootDir,
  );
  await write(
  ...
  await write(
    "packages/storefront-webapp/src/lib/storeConfig.ts",
    "export {};\n",
    rootDir,
  );
  await write(
  ...
  await write(
    "packages/storefront-webapp/src/example.test.tsx",
    "export {};\n",
    rootDir,
  );
  await write(
  ...
  await write(
    "packages/storefront-webapp/tests/e2e/smoke.spec.ts",
    "export {};\n",
    rootDir,
  );
...
describe("generateHarnessDocs", () => {
  it("builds deterministic discovery docs and validation guides for both apps", async () => {
    ...
    expect(
      docs.get("packages/athena-webapp/docs/agent/route-index.md"),
    ).toContain("src/routes/_authed/dashboard.index.tsx");
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/entry-index.md"),
    ).toBeDefined();
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/entry-index.md"),
    ...
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/entry-index.md"),
    ).toContain("app.test.js");
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/entry-index.md"),
    ).toContain("index.js");
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/entry-index.md"),
    ).toContain("test-connection.js");
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/test-index.md"),
    ).toContain("bun run --filter 'valkey-proxy-server' test");
    expect(
      docs.get("packages/valkey-proxy-server/docs/agent/key-folder-index.md"),
    ).toContain("entry-index.md");
    expect(
      docs.get("packages/storefront-webapp/docs/agent/test-index.md"),
    ).toContain("tests/e2e");
    expect(
      docs.get("packages/athena-webapp/docs/agent/key-folder-index.md"),
    ).toContain("src/components");
    expect(
      docs.get("packages/storefront-webapp/docs/agent/validation-guide.md"),
    ).toContain("Full browser journeys");
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-guide.md"),
    ).toContain("src/routeTree.browser-boundary.test.ts");
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-guide.md"),
    ).toContain("Reporting fact ledger, day fold, and read-model edits");
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-guide.md"),
    ).toContain("test -- convex/reports");
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-guide.md"),
    ).toContain(
    ...
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain('"commands"');
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain('"behaviorScenarios"');
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain('"id": "athena.shared-demo-admission"');
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain('"reviewSensitive": true');
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain(
      '"name": "reporting-fact-ledger-day-fold-and-read-model-edits"',
    );
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain("packages/athena-webapp/convex/reports");
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain("packages/athena-webapp/convex/schemas/reports");
    expect(
      docs.get("packages/athena-webapp/docs/agent/validation-map.json"),
    ).toContain("packages/athena-webapp/shared/reportsContract.ts");
    expect(
      docs.get("packages/storefront-webapp/docs/agent/validation-map.json"),
    ).toContain('"kind": "raw"');
