...
describe("HARNESS_APP_REGISTRY", () => {
  ...
  it("documents staff foundation and subsystem credential validation coverage in Athena harness docs", () => {
    const athena = HARNESS_APP_REGISTRY.find(
      (entry) => entry.appName === "athena-webapp"
    );
    const staffFoundationScenario = athena?.validationScenarios.find(
      (scenario) => scenario.title === "Staff foundation and subsystem credential edits"
    );
