...
describe("HARNESS_APP_REGISTRY", () => {
  ...
  it("documents app update readiness and apply-safety validation coverage in Athena harness docs", () => {
    const athena = HARNESS_APP_REGISTRY.find(
      (entry) => entry.appName === "athena-webapp",
    );
    const appUpdateScenario = athena?.validationScenarios.find(
      (scenario) =>
        scenario.title === "App update readiness and apply-safety edits",
    );

    expect(appUpdateScenario?.touchedPaths).toEqual([
      "public/pos-app-shell-sw.js",
      "src/main.tsx",
    ...
    expect(appUpdateScenario?.commands).toEqual([
      {
        kind: "raw",
        command:
    ...
    expect(appUpdateScenario?.note).toBe(
