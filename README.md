...
The tracked Husky hook captures the full pre-push stream in a temporary log, prints a small heartbeat while validation runs, and emits only the pre-push summary on success. On failure it prints the last 200 lines and retains the full log path. This keeps high-volume coverage and browser output from breaking terminal or agent transports without hiding diagnostics or changing the validation exit code.
