...
> **Amendment (2026-08-22): the model narrative is no longer buffered.**
> This plan's exposure semantics said the model-authored narrative is held
> server-side and that only server-authored milestones stream before completion.
> That is superseded by
> [the agent response streaming plan](./2026-08-22-001-feat-agent-response-streaming-plan.md),
> which adds a second exposure rule — **"Provisional stream, recallable view"** —
> under which the narrative streams to the operator live as explicitly
> provisional text. The amended invariant is that the narrative never enters
> Athena's durable record, projected history, a citation, or a prompt, and never
> survives a terminal cause. Release semantics here are unchanged: `completeRun`
> remains the only release, its transaction is untouched, and the committed
> artifact is still the only released answer. The sentences this touches — Key
> Technical Decision 12, the runtime-adapter unit's approach, the "Running"
> host-state row, and the live-region rule — are rewritten in place below; the
> rest of the plan stands as delivered.
...
| Running | Stream batched, server-authored non-sensitive milestones, plus the model narrative as visibly provisional text in a persistent container that says it is unverified, must not be acted on, and will be replaced by the committed answer | Cancel; sources disabled until minted | The milestones live region announces milestones only; the provisional draft has its own polite live region carrying at most one closed-vocabulary cue per draft, never model tokens. A withdrawn draft is removed and announced as an alert. |
