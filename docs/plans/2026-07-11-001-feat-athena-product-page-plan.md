...
- **Make anonymous retries idempotent and non-enumerating:** a high-entropy client submission key and canonical payload digest survive ambiguous retries. Same-key/same-payload retries return accepted. Equivalent same-email submissions using fresh keys receive budget-bounded replay aliases and return accepted while alias capacity remains; exhaustion returns generic recoverable unavailable without persistence. Key reuse with a different payload creates no side effect and returns a generic retry response; materially changed same-email submissions are retained as linked follow-ups rather than discarded.
...
- **How do duplicates and ambiguous retries behave?** Retry the same high-entropy client submission key with the same canonical payload; same-key equivalents remain idempotently accepted. Equivalent submissions with fresh keys create budget-bounded replay aliases so accepted keys cannot later be reused with different content. Once alias capacity is exhausted, another fresh-key equivalent receives generic recoverable unavailable without persistence. Key reuse with different content has no side effect; materially changed same-email requests persist as linked follow-ups even when notification is coalesced.
...
- Before launch, record expected normal and campaign-peak traffic, provider limits, cost ceiling, acceptable rejection risk, and headroom in the abuse-budget report. Derive configurable per-email, global persistence, fresh-key replay-alias, and notification-send ceilings from that record, load-test the selected peak plus headroom, and schedule a threshold review after the first week of traffic. Honeypot submissions return generic accepted without persistence. Same-key equivalent retries remain accepted without spending new capacity. Every newly observed key, including an equivalent fresh-key alias, consumes bounded per-email and global admission capacity; exhaustion returns generic recoverable unavailable without persistence. Notification exhaustion queues accepted records without changing success.
...
- A rapid equivalent repeat with a different key but the same normalized email creates no new request and returns the non-enumerating accepted outcome while replay-alias capacity remains. The alias binds that fresh key to its canonical payload through the replay window; once capacity is exhausted, another fresh-key equivalent returns generic recoverable unavailable without persistence.
...
- Per-email admission, fresh-key replay-alias, global persistence, and notification budgets derived from the approved budget report follow their distinct public outcomes. Same-key retries do not consume new alias capacity; exhausted fresh-key alias capacity returns recoverable unavailable without persistence; notification throttling never reverses durable acceptance.
...
- Each new walkthrough or linked follow-up creates exactly one transactional durable-acceptance milestone; accepted same-key or admitted fresh-key equivalent retries create none, and capacity-rejected fresh-key equivalents create neither a request nor a milestone.
...
- A same-key equivalent duplicate, or a fresh-key equivalent admitted within replay-alias capacity, renders the same confirmation as a new request without revealing prior lead state. Alias-capacity exhaustion retains the form and presents the generic recoverable-unavailable path.
