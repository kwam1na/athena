...
- **Streak + fingerprint alert state on the run table**: alert fires on streak start; re-arm is an explicit write on a clean run (not dedupeKey expiry); a changed difference fingerprint re-alerts once. Partial runs do not clear a mismatch streak unless the previously-differing fields were actually checked and clean. The run row carries a **re-arm epoch counter** (incremented on each re-arm) and a **monotonic `alertSeq`** (incremented on every alert, never reset), both participating in the notification dedupeKey — the rail's dedupe is a permanent unique lookup, so without the epoch a recurring identical discrepancy after a clean run would be silently swallowed, and without `alertSeq` a fingerprint oscillating A→B→A with no intervening clean run (epoch frozen) would rebuild a byte-identical key and be dropped.
...
- Carries: outcome union, explained/unexplained difference summary, fingerprint, streak count, last-alerted fingerprint, re-arm epoch counter (incremented on each clean-run re-arm), monotonic `alertSeq` (incremented on every alert, never reset — both participate in the U5 dedupeKey), verified `certifiedFoldRevision` (absent for pre-stamping legacy days; for week subjects the field stores `reportWeekCurrent.materializedAt`, a documented reuse), timestamps.
...
- Weekly: `verifyCurrentWeekWithCtx` per store, bounded by two gates in order — a **minimum time interval** (`VERIFICATION_WEEK_MIN_INTERVAL_HOURS = 6` against the run row's own `verifiedAt`), then the `materializedAt` advancement check. The time gate is load-bearing: `materializedAt` advances on every rebuild and the 5-minute reports sweep rebuilds active stores' weeks continuously, so the advancement check alone suppresses only idle stores. A pre-flight allocation probe (`VERIFICATION_WEEK_ALLOCATION_PROBE = 1400`) records `truncated` without running verification when the week's read volume would risk the aggregate transaction ceiling — the per-date execution split that removes this cap is follow-up V26-1193.
...
- Runner liveness: per-store consecutive-incomplete counter; operational events (`reports.verification_wedged`) fire on a re-escalation ladder — at the threshold (3 ticks), again at 24 and 168, then every 168 thereafter — re-armed on the first complete tick. A ladder rather than once-per-streak because a permanently wedged store must not go silent after a single missed page.

### Decisions added during delivery (review rounds 1–2)

- Forward stall probe (`VERIFICATION_STALL_PROBE_DAYS = 3`): day selection walks back from the store's current operating date toward the newest folded day, so a fold pipeline that stops producing rows entirely surfaces as missing days. Scope note: any store still folding at all (including an open row for today) breaks the walk immediately — the probe engages only on fully stalled stores.
- Error-row slot cap (`VERIFICATION_ERROR_SLOTS_PER_STORE = 1`, oldest-`verifiedAt` first): contained failures retry every tick but cannot head-of-line-block the day budget; ordinary revision-driven work always keeps a majority of the per-store slots.
- Void/flagged-exclusion attribution context is sourced in `gatherDaySubject` from the day's own `reportFact` rows (`VERIFICATION_VOID_FACT_SCAN = 2000`, the fold's own cap): void magnitudes from `factKind: "void"`, excluded-fact magnitudes from the fold's two exclusion conditions (quarantined, foreign-currency). A capped scan supplies NO attribution — a lower bound used as an explanation ceiling would bless real defects — so an over-cap day alerts rather than being silently explained.
- Missing-day normalization: a `dayStatus: "missing"` result with zero recomputed activity treats the payment-posture comparison as vacuous, so quiet never-folded dates do not alert; any real source activity passes through untouched.
- Runner-health sentinel: the per-store wedge counter lives in `reportVerificationRun` under subjectKind `"week"` with reserved key `__runner_health__` (can never collide with a real `YYYY-MM-DD` cycle key).
- Ledger honesty: crashed selections count as errored subjects and incomplete stores drive the system-scope row's failed counts — a tick that did no work because it crashed must not record `applied`.
...
- Structural dedupeKey over kind + store + subject + fingerprint + re-arm epoch + monotonic `alertSeq` (all from the run row). The rail's dedupe is a permanent unique lookup with no expiry: the epoch lets a re-armed streak with an identical fingerprint alert again, and `alertSeq` keeps an oscillating fingerprint (A→B→A, epoch frozen) from rebuilding a byte-identical key; streak logic in U3 decides when to emit at all.
