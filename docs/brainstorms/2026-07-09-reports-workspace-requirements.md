...
- R77. Recurring non-operational weekdays and date-specific closures must be identified as outside-schedule dates and disclosed whenever reportable activity exists on them. *(Amended 2026-08-02 — see R77a.)*
- R77a. **Amendment, 2026-08-02.** Headline weekly totals cover every date in the resolved seven-date frame, including outside-schedule dates, so the headline answers the period label it is shown under. The outside-schedule portion must be disclosed immediately beneath the headline — naming the amount and the dates — and the scheduled-only figure must remain separately visible, because Daily Close, register variance, and close-derived evidence stay scheduled-day concepts. R77 originally excluded outside-schedule activity from headline totals; live review of the first reporting week showed a headline of GH₵101,933 under a "Jul 27–Aug 2, 2026" label while GH₵31,480 of real sales inside that range sat only in a bottom-of-page disclosure, which read as an inaccurate total rather than a deliberate boundary. R83 is unchanged and still binding: the amount stays separately labeled and is never rolled into an adjacent reporting week.
...
- AE14. **Covers R75-R78, R77a, R83, R84, R84a.** Given the effective Store Schedule starts its reporting cycle Monday, marks Monday through Saturday operational, and marks Sunday non-operational, a Saturday sale after configured closing time remains in the weekly headline, and a Sunday sale is counted in the headline total for the labeled date range while being disclosed immediately beneath it as outside-schedule activity with its date and amount named, and the scheduled-only figure stays separately visible. *(Sunday-appears-only-as-outside-schedule was the pre-2026-08-02 behavior; see R77a.)* If a later effective schedule makes Sunday operational or changes the cycle start, future reports use that definition without rewriting preserved reports.
