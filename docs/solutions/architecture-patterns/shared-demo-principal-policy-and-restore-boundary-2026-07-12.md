...
- Test expired-principal behavior explicitly. An expired demo principal must never fall through as a NORMAL ACTOR. It throws for every read and for every write except one narrow exception added 2026-08-24: a write carrying the `demo.lifecycle` capability and `public: "admit"` — in practice the ticket mint that starts a session — falls through to the `public` adapter, because requiring a live admission in order to obtain one is a deadlock that strands the visitor with no route back. Reads keep throwing, which is what tells the client to renew. See [[architecture-patterns/athena-recovery-must-not-require-what-it-restores-2026-08-24]].
