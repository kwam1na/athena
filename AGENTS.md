...
A release is installed here by one command: `AGENT_SKILLS_CHECKOUT=<agent-skills checkout> bun run agent-skills:install -- <release-id> --profile <profile>`. It builds the release from that checkout, runs the lifecycle update against this repository, and fails unless the lifecycle status sensor then reports `lifecycle: current`, no blockers, and the built release as the active generation. A release install is `sensor-only` work whose declared review scope is `.agent-skills/active.json`; everything else under `.agent-skills/` is generated release payload.

Athena's run-event command is `DELIVERY_EVENT='<json payload>' bun run delivery:emit -- <kind>`, which appends one event through the pinned `@agent-delivery-harness/cli`. The payload travels in `DELIVERY_EVENT` because `bun run` concatenates trailing arguments into a `bash -c` string, where a JSON object is brace-expanded before the CLI sees it. The current run is the pointer the invoking worktree holds in the run store under this repository's git common directory (`managed-delivery/runs/`); `run.started` allocates a run and points this worktree at it, and `--run <id>` names another. Athena's two mandated lens ids are `lens.outcome-correctness` and `lens.adversarial-testing`; every other lens in a round is the executor's own selection.
...
Every file this checkpoint writes lives under the review run root
`<node-tmpdir-realpath>/compound-engineering/execute/<run-id>`, where `<node-tmpdir-realpath>` is the realpath of Node's `os.tmpdir()`. On macOS that is the per-user `$TMPDIR` directory resolved through the `/var` symlink (`/private/var/folders/.../T`) — neither `/tmp` nor the raw `/var/folders/...` form `os.tmpdir()` prints. Print it with `bun -e 'console.log(require("node:fs").realpathSync(require("node:os").tmpdir()))'` and use the printed value literally, including as the manifest's `providerRunRoot`. `bun run harness:review-evidence` resolves the same root and rejects a manifest whose `providerRunRoot` is not its realpath.
...
2. Dispatch the complete relevant reviewer set against that exact tree — passing the ticket's delivery contract via `contract:` — and merge their machine-readable results under the review run root. Do not count implementation subagents as independent reviewers. The complete relevant reviewer set is the always-on layer `ce-code-review` declares — the personas `ce-correctness-reviewer`, `ce-testing-reviewer`, `ce-maintainability-reviewer`, and `ce-project-standards-reviewer`, plus the always-on agents `ce-agent-native-reviewer` and `ce-learnings-researcher` — extended by every cross-cutting, stack-specific, and CE conditional reviewer whose declared selection condition the candidate's diff meets. Review `.agent-skills/**` as generated release payload rather than authored code: check that the payload is internally consistent and that it supports the claims this overlay makes about it, and do not raise style, structure, or refactoring findings against it.
3. If a review fix changes the candidate, run `bun run pr:athena:prepare` again and repeat the complete review on the resulting context. A partial follow-up cannot authorize the changed tree. This checkpoint's first dispatch is review round 1 of the delivery and each repeat is the next round; every one counts against the same delivery-wide bound as the post-validation loop in step 9, and neither stage carries a cap of its own. The installed `review-work` workflow owns that bound and Athena does not narrow it.
4. Only after every required reviewer completes with unanimous approval and zero blocking or unresolved in-scope actionable findings, finalize the run root's `final-manifest.json` with provider `execute`, the context's worktree/candidate fields, reviewer artifacts, findings and mutation sequence, final pass ID, `verdict: "green"`, zero counts, `editedAfterFinalPass: false`, and `finalized: true`. The installed `obtain-review` and `review-work` workflows own which findings are actionable and when a deferral is discharged; record each deferral in the manifest's `findings` with `disposition: "deferred"` and the `deferredIssueId` of its follow-up Linear issue, because the evidence recorder rejects a deferral with no filed issue id and that blocks finalization. Also include the manifest's `reviewLoopTelemetry` block (`iterationCount`, `findingCounts` by severity, `deferredExpansionCount`, `deferredIssueIds`, and `reviewCost` when your agent platform reports what a dispatch consumed) so the same review_iteration/finding-count telemetry posted to Linear also lands repo-side in the obligation record and delivery-run ledger. `reviewCost` carries the platform's own `unit` and `total` plus a `reportedBy` naming that platform — report what it told you, summed across every review round, never an estimate and never converted between units; omit the block when the platform reports nothing.
5. Run `bun run harness:review-evidence -- <review-run-root>/final-manifest.json` with the run root expanded to the resolved path. If review is degraded, times out, exhausts a fix loop, or leaves actionable work, do not finalize or record evidence.
