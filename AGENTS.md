...
3. If a review fix changes the candidate, run `bun run pr:athena:prepare` again and repeat the complete review on the resulting context. A partial follow-up cannot authorize the changed tree. Each repeat is a review round of this delivery and counts against the same delivery-wide bound of three rounds declared in the review and merge loop below; this checkpoint carries no cap of its own.
