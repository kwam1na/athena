...
`<node-tmpdir>/compound-engineering/execute/<run-id>`, where `<node-tmpdir>` is Node's `os.tmpdir()`. On macOS that is the per-user `$TMPDIR` directory (`/var/folders/.../T`), not `/tmp`; print it with `bun -e 'console.log(require("node:os").tmpdir())'` and use the printed value literally. `bun run harness:review-evidence` resolves the same root and rejects a manifest whose `providerRunRoot` is not its realpath.
