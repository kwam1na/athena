...
3. If a review fix changes the candidate, run `bun run pr:athena:prepare` again and repeat the complete review on the resulting context. A partial follow-up cannot authorize the changed tree. This checkpoint's first dispatch is review round 1 of the delivery and each repeat is the next round; every one counts against the same delivery-wide bound of three rounds declared in the review and merge loop below, and this checkpoint carries no cap of its own.
