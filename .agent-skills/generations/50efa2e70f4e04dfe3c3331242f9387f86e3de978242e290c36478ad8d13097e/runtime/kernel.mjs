...
var CanonicalizationError = class extends Error {
  code;
  path;
  constructor(code, path21, message) {
    super(`${message} (at ${path21 === "" ? "the document root" : path21})`);
...
var serializeNumber = (value, path21) => {
...
var serializeString = (value, path21) => {
...
var serialize = (value, path21, ancestors) => {
  if (value === null) return "null";
  switch (typeof value) {
    case "boolean":
      return value ? "true" : "false";
    case "number":
      return serializeNumber(value, path21);
    case "string":
      return serializeString(value, path21);
    case "object":
      break;
    default:
      throw new CanonicalizationError(
  ...
  if (ancestors.has(container)) {
    throw new CanonicalizationError("circular_reference", path21, "value contains itself");
  }
  ...
  try {
    if (Array.isArray(container)) {
      ...
      for (let index = 0; index < source.length; index += 1) {
        elements.push(serialize(source[index], `${path21}/${index}`, ancestors));
      }
    ...
    const members = Object.keys(record2).sort(compareUtf16CodeUnits).map((key) => {
      const name = serializeString(key, path21);
      return `${name}:${serialize(record2[key], `${path21}${pointerSegment(key)}`, ancestors)}`;
    });
    return `{${members.join(",")}}`;
  } finally {
...
function sanitizedCommand(command, label2) {
  ...
  const args = command.map(
    (argument, index) => (
      ...
      redactSecrets(windowed(requireString(argument, `${label2} command argument ${index}`), MAX_BLOCKER_DETAIL_LENGTH))
    )
  );
...
function readStringArray(findings2, member2, value, check) {
  ...
  const out = [];
  let sound = true;
  array.forEach((entry3, index) => {
    const read = readString(findings2, `${member2}[${index}]`, entry3, check);
...
function readObligation(findings2, member2, value) {
  ...
  if (value["allowedResolutionKinds"] !== void 0) {
    ...
    if (array !== void 0) {
      const read = array.map((entry3, index) => readEnum(findings2, `${member2}.allowedResolutionKinds[${index}]`, entry3, RESOLUTION_KINDS));
...
function readAdditionalReviewLenses(findings2, value) {
  ...
  const lenses = [];
  for (const [index, entry3] of entries.entries()) {
    const member2 = `additionalReviewLenses[${index}]`;
    if (!isRecord(entry3)) {
...
function readShape(findings2, input) {
  ...
  if (input["obligations"] !== void 0) {
    ...
    if (array !== void 0) {
      const read = array.map((entry3, index) => readObligation(findings2, `obligations[${index}]`, entry3));
...
function matchesNeutralSet(matchers, path21) {
  return matchers.some((matcher) => path21.startsWith(matcher.prefix) && (matcher.suffix === void 0 || path21.endsWith(matcher.suffix)));
}
...
function parseResolution(value) {
  ...
  if (kind === "evidence") {
    ...
    if (value["checkBinding"] !== void 0) {
      ...
      if (members.some((key) => typeof binding[key] !== "string" || !/^[a-f0-9]{64}$/.test(binding[key]))) throw new RecordShapeError("malformed_shape", "checkBinding requires sha256 digests");
    }
...
function validateChecksPassed(payload, at, provider2, artifacts, context, collector) {
  ...
  try {
    ...
    if (check !== void 0) {
      ...
      const terminal = JSON.parse(context.artifactContents.get("check-result.json") ?? "null");
      outputsMatch = expected?.outputsDigest === digestCanonical(outputs) && artifacts.some((artifact) => artifact.path === "check-result.json" && artifact.role === "check-result") && canonicallyEqual(terminal, { providerId: provider2.id, runId: provider2.runId, finalPassId: provider2.finalPassId, ...payload });
    }
  } catch {
    outputsMatch = false;
  }
...
function retainedCheckOutput(artifactContents, outputPath, index) {
  const text4 = artifactContents.get(`check-output-${index}.json`);
...
function checkFindings(payload, at, collector) {
  ...
  const derived = [];
  const ids = /* @__PURE__ */ new Set();
  findings2.forEach((finding3, index) => {
...
function checkRunHistory(root, collector) {
  ...
  runHistory.forEach((entry3, index) => {
...
function checkArtifacts(root, collector) {
  ...
  const declared = [];
  const seen = /* @__PURE__ */ new Set();
  artifacts.forEach((entry3, index) => {
    ...
    if (isNonEmptyString2(declaredPath) && isNonEmptyString2(sha2562) && isNonEmptyString2(role)) {
      declared.push({ index, path: declaredPath, sha256: sha2562, role });
    }
  });
  return declared;
}
...
function isObligationActive(activation, projection, activationThreshold) {
  ...
  if (binding === void 0) return projection.sensitivePathIds.length > 0;
...
function createArtifactsPort(options = {}) {
  ...
  return {
    ...
    async writeTextFile(target, contents, writeOptions = {}) {
      ...
      const temporary = path4.join(directory, `.${path4.basename(target)}.${process.pid}.${randomUUID3()}.tmp`);
      try {
...
function projectShippedPersonas(read) {
  ...
  for (let index = 0; index < declared.length; index += 1) {
    const entry3 = declared[index];
    const pointer2 = `${PERSONA_MANIFEST_ENTRY}#/personas/${index}`;
...
async function captureCheckOutputs(rootDir, outputs, readOutput) {
  return (await captureCheckOutputSnapshots(rootDir, outputs, readOutput))?.map(({ path: path21, sha256: sha2562 }) => ({ path: path21, sha256: sha2562 }));
}
...
function validateReviewedContext(original, current, outcome) {
  ...
  const candidate2 = binding["candidate"];
...
function parseReviewOutcome(document, charters) {
  ...
  if (unnamed.length > 0) {
    ...
    for (const [index, result2] of unnamed.entries()) {
...
function declaredArtifacts(manifest) {
  ...
  artifacts.forEach((entry3, index) => {
    ...
    entries.push({ index, path: declaredPath, sha256: typeof sha2562 === "string" ? sha2562 : void 0 });
  });
  return entries;
}
...
function verifyPortableEvidence(config, portable, binding, expected, checkBindings) {
  const blockers = [];
...
function verifyOriginalReview(manifest, claim, artifacts, expected) {
  try {
    ...
    validateReviewedContext(original, { spec: "review-context/1", digest: digestCanonical(binding), binding }, rawOutcome);
    ...
    const runHistory = originalRunHistory.map((entry3, index) => index === originalRunHistory.length - 1 ? { ...entry3, preparedTreeSha: manifest.candidate.treeSha } : entry3);
...
async function admit(input, options) {
  ...
  const publishWaiver = options.publishWaiver ?? ((rootDir, binding2, obligationId, resolution) => publishRecord(rootDir, { gateId: input.config.gateId, obligationId, candidateBinding: binding2, resolution }, storageOptions2(options)));
  const binding = recordBindingOf(candidate2);
  const published = [];
  const grantedIds = [];
  for (const obligationId of covered) {
    ...
    const record2 = await publishWaiver(input.rootDir, binding, obligationId, {
      kind: "waiver",
...
function parseDeliveryRecord(text4) {
  ...
  if (!isRecord7(binding)) return malformed("missing candidateBinding");
  for (const field of BINDING_FIELDS) {
    if (!isNonEmptyString3(binding[field])) return malformed(`candidateBinding is missing ${field}`);
  }
...
function verifyDeliveryRecord(config, record2, recomputedIdentity, base, options = {}) {
  ...
  const binding = record2.candidateBinding;
...
var closedArray = (rules, options = {}) => (value, at, collector) => {
  ...
  value.forEach((entry3, index) => {
    checkClosed2(entry3, spinePointer(at, index), rules, collector);
  });
};
...
function checkOutcomeCoversContract(outcome, contract2) {
  ...
  contract2.acceptanceCriteria.forEach((criterion, index) => {
...
function checkContractWithinPolicy(contract2, policy) {
  ...
  contract2.requestedAuthority.forEach((authority, index) => {
...
function createProviderReviewHandoff(input) {
  ...
  return { spec: PROVIDER_REVIEW_HANDOFF_SPEC, ...binding, promptContextBytes, promptContextDigest, reviewer: reviewer2 };
}
...
async function readMaintenanceJournal(installationPath) {
  ...
  for (const [index, line] of text4.split("\n").filter((candidate2) => candidate2.length > 0).entries()) {
    ...
    if (!verdict.ok) {
      return {
        ok: false,
        message: `maintenance journal line ${index} is outside the frozen grammar: ${verdict.rejections.map((rejection) => rejection.message).join("; ")}`
      };
    }
    const record2 = parsed;
    if (record2["journal"] !== "maintenance") {
      return { ok: false, message: `maintenance journal line ${index} belongs to the ${String(record2["journal"])} journal` };
    }
    entries.push({
...
function interruptedMaintenanceOf(entries) {
  ...
  for (const entry3 of entries) {
    ...
    } else if ((phase === "completed" || phase === "recovered") && entry3.payload["action"] === open8?.action) {
      open8 = void 0;
    }
  }
  return open8;
...
async function materializeRoot(packedDir, root) {
  ...
  for (const directory of ordered) {
    ...
    await chmod7(directory, READONLY_DIR);
  }
...
async function removeReadonlyTree(root) {
  const { chmod: chmod11 } = await import("node:fs/promises");
  const restore = async (dir) => {
    await chmod11(dir, 448).catch(() => void 0);
    let entries;
    try {
...
var closedArrayWithOptionals = (required, collectorRef, optional = []) => (value, at) => {
  ...
  value.forEach((entry3, index) => {
    checkClosedWithOptionals(entry3, spinePointer(at, index), required, optional, collectorRef);
  });
};
...
import { chmod as chmod8, mkdir as mkdir8, open as open5, truncate } from "node:fs/promises";
...
async function readRawJournal(journalPath, discipline = {}) {
  ...
  try {
    ...
    if (discipline.refuseOnError === true) throw new JournalAccessRefused(journalPath, describe4(error));
    return EMPTY_RAW;
  }
  try {
    const refusal2 = discipline.verify?.(await handle.stat());
...
`, "utf8");
    ...
    await chmod8(journalPath, OWNER_FILE5);
...
function applySecretDiscipline(entry3, freeTextMembers = FREE_TEXT_MEMBERS) {
  ...
  const walk = (value, pointer2, member2) => {
    ...
    if (Array.isArray(value)) {
      return value.map((item, index) => walk(item, `${pointer2}/${index}`, void 0));
    }
    if (typeof value === "object" && value !== null) {
      const copy = {};
...
var RUN_FREE_TEXT_MEMBERS = /* @__PURE__ */ new Set(["rationale", "summary", "choice", "cited", "fork"]);
...
var contract = (value, at, collector) => {
  checkClosed2(value, at, [
    ...
    { name: "acceptanceCriteria", check: (items, pointer2, c) => {
      ...
      items.forEach((item, index) => freeText(item, spinePointer(pointer2, index), c));
    } }
  ], collector);
};
...
function envelopeMembers(withSeq) {
  return [
    { name: "version", check: oneOf2([RUN_EVENT_SPEC]) },
...
function validateRunEvent(value, options = {}) {
  ...
  checkClosed2(value, "", envelopeMembers(withSeq), collector);
  const payload = value["payload"];
  checkClosed2(payload, "/payload", PAYLOAD_MEMBERS2[kind], collector);
  if (isSpineRecord(payload)) {
...
import { mkdir as mkdir9, open as open6, readdir as readdir4, unlink } from "node:fs/promises";
...
`, "utf8");
  ...
  const readJournalFile = async (journalPath) => {
    ...
    const events = [];
    for (const [index, entry3] of parsed.entries.entries()) {
  ...
  return {
    ...
    async discard(runId) {
      ...
      const notePath = notePathFor(runId);
      if (notePath !== void 0) await unlink(notePath).catch(() => void 0);
      return { ok: true };
    },
    ...
    async setCurrent(worktreeKey, runId, options) {
      ...
      return serializedOnPath(pointerPath, async () => {
        ...
        await unlink(pointerPath).catch(() => void 0);
...
import { chmod as chmod9, mkdir as mkdir10, readFile as readFile11, rm as rm6, writeFile as writeFile7 } from "node:fs/promises";
...
async function writeOwned(target, contents) {
  ...
  await chmod9(target, OWNER_FILE6);
}
...
import { lstat, open as open7, readFile as readFile12, realpath as realpath4, rename as rename5, rm as rm7, stat as stat5, writeFile as writeFile8 } from "node:fs/promises";
...
async function writeEntry(input, record2, lockPath) {
  ...
  const existing = index >= 0 ? deliveries[index] : {};
  ...
  if (index >= 0) deliveries[index] = entry3;
...
`);
    await rename5(tempPath, input.gateRecordPath);
  } catch (error) {
...
function composeBlockerInventory(views) {
  ...
  const resolve = (indices) => {
    for (const index of indices) {
      entries[index] = { ...entries[index], resolved: true };
    }
  };
  ...
  return async (requested) => {
    ...
    for (let depth = 0; depth < 32; depth += 1) {
      ...
      for (let index = 0; index < segments.length; index += 1) {
        ...
        current = path19.posix.normalize(path19.posix.join(path19.posix.dirname(prefix), target, ...segments.slice(index + 1)));
...
async function capturePortableVerificationInputs(rootDir, config, candidate2, record2, run = runGitCommand) {
  ...
  const checkBindings = await captureCheckBindings(rootDir, config, candidate2, {
    ...
    readOutput: async (repoPath, providerId2) => {
      ...
      const index = config.providers.find((provider2) => provider2.id === providerId2)?.check?.outputs?.indexOf(repoPath) ?? -1;
...
import { chmod as chmod10, mkdir as mkdir11, readFile as readFile13, readdir as readdir5, realpath as realpath5, rm as rm8, stat as stat6, writeFile as writeFile9 } from "node:fs/promises";
...
var compiledAdopterPolicyBindingDigest = (binding) => digestCanonical(binding);
...
async function writeOwned2(target, contents) {
  ...
  await chmod10(target, OWNER_FILE7);
}
...
async function voidSupersededBindingStates(bindingDir, currentFence) {
  let names;
  try {
    names = await readdir5(bindingDir);
  } catch {
    return;
  }
  for (const name of names) {
...
function createManagedDeliveryFacade(input) {
  ...
  const guard = async (deliveryId, options = {}) => {
    ...
    if (binding.ok && binding.activeCompositionProfile === "confirmation-fixture" && !(binding.disposableRepositoryIds ?? []).includes(meta.contract.repository.repositoryId)) {
      return refuse3(
...
`
    ...
    async status({ deliveryId, observedAt }) {
      ...
      const resume = terminal ? "none" : activity === "active" ? "none" : sameWorkspaceResumable ? "same-workspace" : "takeover-required";
    ...
    async submitStageResult({ deliveryId, stageId, resultBytes, fence }) {
      ...
      if (binding === void 0 || binding.stageId !== stageId) {
...
`
    ...
    async recoverSecurityBlocked({ deliveryId, targetGenerationDigest, assertionSource, now }) {
      ...
      try {
        await readdir5(attemptsDir);
        const { rename: rename6 } = await import("node:fs/promises");
        await rename6(attemptsDir, path20.join(dir, `attempts-invalidated-r${reduced.state.expectedRevision}`));
      } catch {
      }
...
function outcomeBlockers(providerId2, consumption) {
  ...
  return [
    railBlocker(
      providerId2,
      consumption.status,
      terminal?.summary ?? `Provider ${providerId2} ended ${consumption.status}.`,
      terminal?.details,
      terminal?.action
    )
  ];
}
function manifestPathOf(terminal) {
