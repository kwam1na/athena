...
function parseRunExport(text) {
  const invalid = { ok: false, code: "run_export_invalid" };
  try {
    const value = JSON.parse(text);
    if (!isRecord(value) || value["spec"] !== "delivery-run-export/1" || value["labels"] !== READOUT_LABELS || typeof value["runId"] !== "string" || value["runId"].length > 128 || !RUN_STORE_ID2.test(value["runId"]) || !Array.isArray(value["events"]) || !Array.isArray(value["refusedAppends"]) || !isRecord(value["readout"])) return invalid;
    ...
    const expected = buildRunExport({ runId: value["runId"], events: value["events"] });
    if (canonicalize(value["summary"]) !== canonicalize(expected.summary) || canonicalize(value["costs"]) !== canonicalize(expected.costs)) return invalid;
    const { note, ...readout } = value["readout"];
    if (note !== void 0 && typeof note !== "string" || canonicalize(readout) !== canonicalize(expected.readout)) return invalid;
    return { ok: true, value: {
      ...expected,
      refusedAppends: value["refusedAppends"],
      readout: { ...expected.readout, ...note === void 0 ? {} : { note } }
    } };
  } catch {
    return invalid;
  }
