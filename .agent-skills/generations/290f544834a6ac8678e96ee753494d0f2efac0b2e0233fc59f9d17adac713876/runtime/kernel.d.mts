...
interface ScopedCheckDefinition {
    ...
    readonly environment: readonly {
        ...
        readonly kind: "flag" | "credential";
    }[];
}
declare function isScopedCheckDefinition(value: unknown): value is ScopedCheckDefinition;
/** Supported private executor profiles; old runtimes reject this opt-in. */
interface ScopedExecutionProfile {
...
interface ScopedExecutionDefinition {
...
declare function selectScopedCheckAttempt(providerId: string, inputDigest: string, profileDigest: string, attempts: readonly ScopedCheckAttempt[]): ScopedCheckAttempt | undefined;
/**
 ...
 */
declare const TRACKER_POSTURES: readonly ["available", "degraded", "absent"];
type TrackerPosture = (typeof TRACKER_POSTURES)[number];
/**
 ...
 */
declare function trackerPostureOf(adapters: readonly AdapterCapability[]): TrackerPosture;
/**
 ...
 */
declare const RUN_EVENT_PAYLOAD_GRAMMAR_SPEC = "run-event-payload-grammar/2";
type RunEventValueType = "string" | "number" | "boolean" | "array" | "object";
/** One value's described shape: what the validator accepts, and one value it does. */
interface RunEventValueGrammar {
...
interface RunEventPayloadMemberGrammar extends RunEventValueGrammar {
    readonly name: string;
 ...
 */
declare const RUN_JOURNAL_ROUND_BINDINGS: readonly ["record-tree", "reviewed-tree", "unbound"];
type RunJournalRoundBinding = (typeof RUN_JOURNAL_ROUND_BINDINGS)[number];
/**
 ...
 * it is.
 */
interface RunJournalExplanation {
 ...
 */
interface RunJournalDiagnostics {
 ...
 */
declare function explainRunJournal(events: readonly RunEvent[], treeSha?: string, mandatedLensIds?: readonly string[], reviewedTreeShas?: readonly string[]): RunJournalDiagnostics;
 ...
 */
interface DeliveryRecordRunSpan {
    readonly startedAt: string;
    readonly endedAt: string;
}
/** Both instants well formed, in order, and nothing else in the member. */
declare function isDeliveryRecordRunSpan(value: unknown): value is DeliveryRecordRunSpan;
 ...
 */
declare const CONTROL_PLANE_CLAIM_KINDS: readonly ["enqueued", "advanced", "completed", "cancelled", "approval-notified", "host-start-requested"];
/** How the local delivery reconciled the claim. Three outcomes, no fourth. */
declare const CONTROL_PLANE_DISPOSITIONS: readonly ["mirror-only", "blocker", "coalesced"];
/**
...
declare const SUPPORTED_CONTRACT_VERSIONS: Readonly<{
    ...
    readonly controlPlane: "control-plane-coordination/1";
}>;
...
interface HostSessionGrant {
    readonly allowedCapabilities: readonly string[];
    readonly writablePaths: readonly string[];
    readonly protectedPaths: readonly string[];
}
interface ComposeHostSessionInput {
...
interface HostBindingRefusalDetail {
    readonly code: string;
    readonly message: string;
}
type ComposeHostSessionResult = ComposedHostSession | {
    readonly ok: false;
    readonly blockers: readonly HostBindingRefusalDetail[];
};
/**
 * One host's delivery binding. The facade holds a single instance and never
 * chooses between instances.
 */
interface ManagedHostBinding {
 ...
 * allowed to exist on.
 */
declare const claudeCodeBinding: ManagedHostBinding;
 ...
 */
declare const CODEX_HOOK_EXECUTION_MODE = "sync";
declare const CODEX_THREAD_CONFIG_FILE: (fence: number) => string;
/**
 ...
 * string.
 */
declare const codexPermissionProfileId: (workspaceRoot: string, fence: number) => string;
/**
 ...
 */
declare const CODEX_UNENFORCEABLE_TOOL_SOURCES: readonly ["mcp", "app", "hosted", "dynamic", "plugin"];
type CodexUnenforceableToolSource = (typeof CODEX_UNENFORCEABLE_TOOL_SOURCES)[number];
/**
 ...
 */
declare const CODEX_DISABLED_FEATURE_KEYS: readonly ["tool_registry", "multi_agent_v2", "web_search"];
type CodexDisabledFeatureKey = (typeof CODEX_DISABLED_FEATURE_KEYS)[number];
/**
...
declare const CODEX_HOST_TOOLS: readonly CodexHostTool[];
declare const codexHostTool: (hostName: string) => CodexHostTool | undefined;
/**
 ...
 */
declare const codexEnabledToolsFor: (allowedCapabilities: readonly string[]) => readonly string[];
/**
 ...
 */
declare const codexEscalationTokens: (kind: string) => readonly string[];
/**
 ...
 */
declare const codexNamesDisabledSurface: (kind: string) => boolean;
/**
 ...
 */
declare const codexSubagentPosture: () => {
    readonly capability: "removed";
    readonly reason: string;
};
 ...
 */
declare function codexPermissionProfileOf(input: {
    readonly workspaceRoot: string;
    readonly commonGitDir: string;
    readonly authorityDir: string;
    readonly fence: number;
 ...
 */
interface CodexThreadStartRequest {
...
interface ComposeCodexAppServerThreadResult {
    readonly ok: true;
 ...
 * on its own.
 */
declare const CODEX_HOOK_SUBCOMMAND = "codex-pre-tool-use";
/**
 ...
 */
declare function composeCodexAppServerThread(input: ComposeHostSessionInput): Promise<ComposeCodexAppServerThreadResult>;
declare const CODEX_APPLIED_MISMATCH_CODES: readonly ["permission_profile_mismatch", "sandbox_mode_mismatch", "writable_roots_mismatch", "deny_write_roots_mismatch", "deny_read_roots_mismatch", "network_access_enabled", "ambient_temp_not_excluded", "hook_missing", "hook_command_mismatch", "hook_not_synchronous", "unenforceable_tool_surface_enabled", "disabled_feature_enabled", "configuration_digest_mismatch"];
type CodexAppliedMismatchCode = (typeof CODEX_APPLIED_MISMATCH_CODES)[number];
/**
 ...
 */
interface CodexAppliedThreadConfiguration {
...
interface CodexAppliedMismatch {
...
type VerifyAppliedCodexThreadConfiguration = {
    ...
    readonly mismatches: readonly CodexAppliedMismatch[];
};
/**
 ...
 */
declare function verifyAppliedCodexThreadConfiguration(expected: ComposeCodexAppServerThreadResult, applied: CodexAppliedThreadConfiguration): VerifyAppliedCodexThreadConfiguration;
declare const CODEX_ESCALATION_REFUSAL_CODES: readonly ["session_wide_permission_widening_refused", "exec_policy_amendment_refused", "extra_writable_root_refused", "unenforceable_tool_source_refused"];
type CodexEscalationRefusalCode = (typeof CODEX_ESCALATION_REFUSAL_CODES)[number];
interface CodexEscalationRequest {
 ...
 */
declare function evaluateCodexEscalation(request: CodexEscalationRequest, profile: CodexPermissionProfile): {
    readonly refused: false;
} | {
    readonly refused: true;
    readonly code: CodexEscalationRefusalCode;
    readonly message: string;
};
/**
 ...
 */
declare const codexAppServerBinding: ManagedHostBinding;

interface HookBindingState {
...
declare const CODEX_HOOK_EVENT_NAME = "PreToolUse";
/**
 ...
 */
declare function codexToolIsLocallyEnforceable(toolName: string, toolSource?: unknown): boolean;
/**
 ...
 * "writes nothing".
 */
declare function decideCodexHookInvocation(state: HookBindingState | undefined, input: CodexHookInput, observedAt: string, sessionFence: number): HookDecision;
/**
 ...
 */
declare function codexWrittenPaths(members: readonly string[], toolInput: Record<string, unknown> | undefined): readonly string[];
/**
 ...
 */
declare function renderCodexHookDecision(decision: HookDecision): string;
/**
 ...
 */
declare function parseCodexHookInput(raw: string): CodexHookInput | undefined;
/**
 ...
 */
declare function codexHookTurn(input: {
 ...
 */
declare function codexHookDecision(input: {
    readonly state: HookBindingState | undefined;
    readonly rawInput: string;
    readonly observedAt: string;
    readonly sessionFence: number;
...
interface CodexAppServerConformancePortInput {
    readonly worktreeDir: string;
    readonly generationRoot: string;
    readonly bindingDir: string;
    readonly deliveryId: string;
 ...
 */
declare function faithfullyAppliedCodexConfiguration(composed: ComposeCodexAppServerThreadResult): CodexAppliedThreadConfiguration;
declare function createCodexAppServerConformancePort(input: CodexAppServerConformancePortInput): HostIntegrationPort;
...
declare const ACTION_APPROVAL_ORIGIN_PREFIX = "action-approval:";
/**
...
declare function actionChainDigest(chain: readonly ActionChainLink[], next: ExternalAction): string;
 ...
 */
declare function evaluateActionApproval(assertion: Record<string, unknown>, context: ActionApprovalContext): ActionApprovalVerdict;
/** The adapter the action will be invoked through. No credential travels here. */
interface ActionAdapterBinding {
    readonly capabilityId: string;
    readonly kind: PolicyCapabilityKind;
...
interface PlanExternalActionInput {
    readonly deliveryId: string;
    readonly intentId: string;
    readonly action: ExternalAction;
    readonly contract: AcceptedContract;
 ...
 */
declare function planExternalAction(input: PlanExternalActionInput): PlanExternalActionVerdict;
/** Exactly the frozen `action.intent.recorded` payload, and nothing else. */
declare function journalIntentPayload(intent: BoundActionIntent): ExternalActionIntent;
 ...
 */
declare function revalidateBeforeInvoke(intent: BoundActionIntent, observed: RevalidationObservation): {
    readonly ok: true;
} | {
    readonly ok: false;
    readonly refusals: readonly FinishLineRefusal[];
...
interface InvokeExternalActionInput {
    readonly intent: BoundActionIntent;
 ...
 */
declare function invokeExternalActionOnce(input: InvokeExternalActionInput): Promise<InvocationOutcome>;
/** Exactly the frozen `action.result.recorded` payload, and nothing else. */
declare function journalResultPayload(result: ObservedActionResult): {
    readonly intentId: string;
    readonly action: ExternalAction;
    readonly outcome: ExternalActionOutcome;
    readonly verification: ActionVerification;
...
declare const RECONCILIATION_FINDINGS: readonly ["performed", "not-performed", "unknown"];
type ReconciliationFinding = (typeof RECONCILIATION_FINDINGS)[number];
 ...
 * refusal.
 */
declare function reconcileBeforeRetry(input: ReconcileInput): ReconciliationDisposition;
 ...
 */
declare function classifyActionOutcome(input: ClassifyActionInput): ActionClassification;
interface DeployPreconditionInput {
    ...
    readonly provenance: {
        ...
        readonly candidateDeliverableDigest: string;
    };
 ...
 */
declare function checkDeployPreconditions(input: DeployPreconditionInput): {
    readonly ok: true;
} | {
    readonly ok: false;
    readonly refusals: readonly FinishLineRefusal[];
 ...
 */
declare const EXTERNAL_ACTION_RESULT_SPEC = "external-action-result/1";
declare function validateExternalActionResult(value: unknown): SpineVerdict;
...
declare const COORDINATION_MESSAGE_SPEC = "coordination-message/1";
/**
 ...
 */
declare const COORDINATION_PROTOCOL_VERSION = "control-plane-coordination/1";
/**
 ...
 */
declare const COORDINATION_MESSAGE_KINDS: readonly ["enqueue", "mirror", "host.start.requested", "approval.notified", "terminal.projection"];
type CoordinationMessageKind = (typeof COORDINATION_MESSAGE_KINDS)[number];
/**
 ...
 */
declare const CONTROL_PLANE_CLAIMS: readonly ["enqueued", "advanced", "completed", "cancelled", "approval-notified", "host-start-requested"];
type ControlPlaneClaim = (typeof CONTROL_PLANE_CLAIM_KINDS)[number];
/**
 ...
 */
declare function validateCoordinationMessage(value: unknown): SpineVerdict;
 ...
 */
declare const COORDINATION_REFUSALS: readonly ["message_malformed", "protocol_unsupported", "channel_unrecognized", "trust_root_confusion", "repository_scope_mismatch", "delivery_scope_mismatch", "nonce_replayed", "sequence_regressed", "authority_not_local"];
type CoordinationRefusalCode = (typeof COORDINATION_REFUSALS)[number];
interface CoordinationRefusal {
 ...
 */
interface CoordinationAdmissionView {
    readonly repositoryId: string;
    readonly deliveryId: string;
...
type CoordinationAdmission = {
    ...
    readonly ok: false;
    readonly refusals: readonly CoordinationRefusal[];
};
declare function admitCoordinationMessage(value: unknown, view: CoordinationAdmissionView): CoordinationAdmission;
/**
 ...
 */
declare function replayLedgerOf(records: readonly MirroredMessageView[], channelDigest: string): CoordinationReplayLedger;
 ...
 * codes.
 */
declare const CONTROL_PLANE_CONFLICT_BLOCKER_CODE = "control-plane.claim-contradicted";
interface LocalHistoryView {
    /**
 ...
 * append.
 */
type ClaimDisposition = "mirror-only" | "blocker" | "coalesced";
interface ClaimReconciliation {
 ...
 */
declare function claimContradictsLocalHistory(message: CoordinationMessage, local: LocalHistoryView): boolean;
declare function reconcileRemoteClaim(message: CoordinationMessage, local: LocalHistoryView): ClaimReconciliation;
/**
 ...
 */
declare function localFactEpochOf(entryKinds: readonly string[], isObservationOnly: (kind: string) => boolean): number;
/**
 ...
 */
declare function conflictBlockerEpochOf(records: readonly MirrorRecordView[]): number;
...
interface CoordinationDispatch {
    readonly ok: boolean;
    readonly code?: string;
    readonly message: string;
}
...
declare const COORDINATION_PORT_UNBOUND_CODE = "coordination_port_unbound";
declare const UNBOUND_COORDINATION_PORT: CoordinationPort;
...
interface SimulatedMessageOptions {
    /**
...
declare const SIMULATED_MESSAGE_OPTION_NAMES: readonly (keyof SimulatedMessageOptions)[];
interface CoordinationSimulatorOptions {
    readonly repositoryId: string;
    readonly deliveryId: string;
    readonly keyId: string;
    readonly channelDigest: string;
...
interface CoordinationSimulator extends CoordinationPort {
    /**
...
declare function createCoordinationSimulator(options: CoordinationSimulatorOptions): CoordinationSimulator;
...
interface ListedDelivery {
    ...
    readonly lastActivity: {
        readonly activity: HostActivityState;
 ...
 */
declare const DEFAULT_OBSERVATION_LIFETIME_SECONDS = 3600;
/**
 ...
 */
declare const resolveObservationLifetimeSeconds: (declaredSeconds: number | undefined) => number;
/** The freshness heartbeat the binding rewrites on every allowed PreToolUse. */
interface HostObservationStamp {
    readonly fence: number;
    readonly observedAt: string;
}
/** The last `activity.observed` journal entry, reduced to what the grade reads. */
interface ObservedActivityEntry {
    readonly activity: HostActivity;
    readonly fence: number;
}
interface GradeHostActivityInput {
 ...
 */
declare const gradeHostActivity: (input: GradeHostActivityInput) => HostActivity;
...
interface CandidateTreeInputMetadata {
...
type CandidateTreeInputReader = ReviewInputReader & {
    metadata(repoPath: string): Promise<CandidateTreeInputMetadata>;
};
declare function candidateTreeEvidenceReader(rootDir: string, treeSha: string, run?: CandidateCommandRunner): Promise<CandidateTreeInputReader>;
...
declare function candidateTreeSourceReader(rootDir: string, treeSha: string, run?: CandidateCommandRunner): Promise<CandidateTreeInputReader>;
...
interface ScopedInputCapturePorts {
    ...
    readonly readMetadata?: (repoPath: string) => Promise<CandidateTreeInputMetadata>;
...
interface ScopedInputCapture {
    ...
    readonly files: readonly {
        readonly path: string;
        readonly sha256: string | null;
        readonly metadata?: CandidateTreeInputMetadata;
    }[];
...
declare function captureScopedCheckInputs(definition: ScopedCheckDefinition, ports: ScopedInputCapturePorts): Promise<ScopedInputCapture>;
...
interface ScopedRuntimeObservation {
...
declare function scopedCheckIdentity(config: HarnessConfig, provider: ProviderRegistration, inventory: readonly string[], read: CandidateTreeInputReader, observation: ScopedRuntimeObservation, candidate: Pick<CapturedCandidate, "base" | "headSha" | "treeSha">, readEvidence?: ReviewInputReader): Promise<{
    profileDigest: string;
    version: "scoped-inputs/1";
    inputDigest: string;
    reusable: boolean;
    files: readonly {
        readonly path: string;
...
export { ABSENT_BY_STATE, ACTION_APPROVALS, ACTION_APPROVAL_ORIGIN_PREFIX, ACTION_VERIFICATIONS, ACTIVATION_KINDS, ACTIVE_POINTER_SPEC, ADAPTER_CAPABILITY_SPEC, ADMISSION_DENIAL_CODES, APPROVAL_REQUEST_KINDS, APPROVAL_REQUIREMENTS, ARCHIVE_RELEASE_MANIFEST_ENTRY, ARTIFACT_OBSERVATION_STATUSES, ASSERTION_CLASSES, ASSERTION_PROVIDER_SPEC, ASSERTION_SOURCES, ATTESTATION_LABEL, ATTESTATION_LEVELS, AUTHORITY_REVOCATION_SPEC, BASE_MOVEMENT_POLICIES, BLOCKER_CONTRACT_VERSION, BLOCKER_SOURCE_KINDS, BlockedError, CANDIDATE_CAPTURE_CODES, CANDIDATE_DIFF_UNREADABLE, CANDIDATE_DRIFT_CLASSES, CANDIDATE_MODES, CANDIDATE_PATH_CLASSES, CANDIDATE_VCS, CAPABILITY_DESCRIPTOR_SPEC, CAPABILITY_KINDS, CLAUDE_SKILL_EXPOSURE_PREFIX, CODEX_APPLIED_MISMATCH_CODES, CODEX_APP_SERVER_HOST_ID, CODEX_CHARACTERIZED_HOST_VERSION, CODEX_DISABLED_FEATURE_KEYS, CODEX_ESCALATION_REFUSAL_CODES, CODEX_HOOK_EVENT, CODEX_HOOK_EVENT_NAME, CODEX_HOOK_EXECUTION_MODE, CODEX_HOOK_SUBCOMMAND, CODEX_HOST_TOOLS, CODEX_PINNED_HOST_VERSION, CODEX_THREAD_CONFIG_FILE, CODEX_UNENFORCEABLE_TOOL_SOURCES, COMPILED_POLICY_SPEC, COMPOSITION_MANIFEST_FILE, COMPOSITION_MANIFEST_SPEC, COMPOSITION_PROFILES, CONFIG_FINDING_CODES, CONFIRMATION_CLASSES, CONFIRMATION_DENIAL_CODES, CONFIRMATION_FIXTURE_PROFILE, CONFIRMATION_OPERATION_PREFIX, CONFORMING_ATTESTATION_LEVEL, CONSUMPTION_GATE_RECORD_BLOCKER_CODES, CONTROL_PLANE_CLAIMS, CONTROL_PLANE_CLAIM_KINDS, CONTROL_PLANE_CONFLICT_BLOCKER_CODE, CONTROL_PLANE_DISPOSITIONS, COORDINATION_MESSAGE_KINDS, COORDINATION_MESSAGE_SPEC, COORDINATION_PORT_UNBOUND_CODE, COORDINATION_PROTOCOL_VERSION, COORDINATION_REFUSALS, CRITERION_DISPOSITIONS, CanonicalizationError, DEFAULT_BASE_MOVEMENT_POLICY, DEFAULT_BASE_REF, DEFAULT_CAPTURE_ATTEMPTS, DEFAULT_OBSERVATION_LIFETIME_SECONDS, DEFAULT_RUN_FRESHNESS_WINDOW_MS, DEFAULT_STORAGE_NAMESPACE, DELIVERABLE_TREE_V1, DELIVERABLE_TREE_V1_NARRATION_SET, DELIVERY_BLOCKER_REMEDIATIONS, DELIVERY_EVIDENCE_1, DELIVERY_OWNED_TREE_PREFIXES, DELIVERY_PROVIDER_RAILS_VERSION, DELIVERY_RECORD_DRIFT_CLASSES, DELIVERY_RECORD_VERSION, DELIVERY_STATES, DELIVERY_TRANSITION_TABLE, DESCENDANT_TEARDOWN_STATUSES, DISPOSABLE_OUTCOME_AUTHORITIES, DISPOSABLE_REVIEW_LENSES, DISPOSABLE_SENSOR_CAPABILITY, DISPOSABLE_STAGE_GRANT, DigestError, EVENT_VOCABULARY, EVIDENCE_KINDS, EXECUTION_CONTEXT_KINDS, EXECUTION_GRANT_SPEC, EXTERNAL_ACTIONS, EXTERNAL_ACTION_OUTCOMES, EXTERNAL_ACTION_RESULT_SPEC, EXTERNAL_VERIFICATIONS, FACADE_CAPABILITY_CLASSES, FACADE_OPERATIONS, FACADE_SURFACES, FINDING_DISPOSITIONS, FINDING_SCOPES, FINDING_SEVERITIES, FINISH_LINES, FINISH_LINE_ACTIONS, FINISH_LINE_RESULT_SPEC, FRESHNESS_KINDS, GATE_STRUCTURAL_FINDING_CODES, GRANT_ATTESTATION_SPEC, GRANT_PROFILES, HARNESS_VERSION, HOST_ACTIVITY_STATES, HOST_ADMISSION_SCENARIOS, HOST_BINDING_BLOCKER_CODES, HOST_CONFORMANCE_CASES, HOST_INTERCEPTION_SCENARIOS, IDENTITY_DOMAIN, IDENTITY_FINDING_CODES, INSTALL_RECEIPT_SPEC, INTAKE_STATES, INTERNAL_ERROR_CODE, INVOCATION_FENCE_SPEC, INVOCATION_WAIVER_SCOPE, JOURNALS, JOURNAL_ENTRY_SPEC, MANAGED_DELIVERY_NAMESPACE, MANDATORY_LENS_CATEGORIES, MANIFEST_REJECTION_CODES, MANIFEST_REJECTION_REGISTRY, MANIFEST_RULE_IDS, MAX_BLOCKER_DETAIL_LENGTH, MAX_PORTABLE_ARTIFACTS, MAX_PORTABLE_ARTIFACT_BYTES, MAX_PORTABLE_EVIDENCE_BYTES, MAX_PORTABLE_RECORD_BYTES, MAX_RUN_LABEL, MAX_RUN_LENSES, MAX_RUN_PATH, MAX_RUN_PROVIDER_ID, MAX_RUN_URL, META_RULE_IDS, MINIMUM_NODE, MINIMUM_PYTHON, NON_WAIVABLE_INTEGRITY_CODES, OBSERVATION_ONLY_KINDS, OPERATION_CLAIM_SPEC, OPERATION_RESULT_SPEC, OPERATOR_CONFIRMATION_SPEC, OTHER_INSTALLATION_ARTIFACTS, OUTCOME_VERIFICATION_SPEC, PACKAGE_NAME, PACKED_HARNESS_PACKAGES, PATH_MATCHER_KINDS, PERSONA_MANIFEST_ENTRY, PERSONA_MANIFEST_SPEC, PINNED_AGENT_SKILLS, POLICY_CAPABILITY_KINDS, POLICY_COMPILE_CODES, POLICY_SNAPSHOT_SPEC, PORTABLE_INTAKE_GRANT, PORTABLE_MODEL_DRIVEN_STAGES, PORTABLE_PRIVILEGED_CREDENTIALS, PORTABLE_STAGE_GRANT, PREPARATION_FAILURE_CLASSES, PREPARATION_RECEIPT_LEAF, PREPARATION_RECEIPT_SCHEMA_VERSION, PRIVILEGED_ACTIONS, PRIVILEGED_CAPABILITY_KINDS, PRODUCT_COMPOSITION_PIN_SPEC, PRODUCT_TRUST_LABEL, PRODUCT_TRUST_STATE_SPEC, PROJECTION_CONSUMPTION_OBSERVATION_SPEC, PROJECTION_DIR, PROJECTION_RECEIPT_FILE, PROVIDER_POLICIES, PROVIDER_REVIEW_HANDOFF_SPEC, PROVIDER_REVIEW_RESULT_SPEC, ProcessLockRefused, READ_ONLY_CAPABILITY_KINDS, RECEIPTED_SKILLS_ROOT, RECHECKED_VALUES, RECONCILIATION_FINDINGS, RECORDER_EMITTED_CODES, RECORDS_LEAF, RECORD_QUARANTINE_REASONS, RECORD_SCHEMA_VERSION, REPOSITORY_POLICY_DOCUMENT_SPEC, RESOLUTION_KINDS, RESOLUTION_OUTCOMES, RESUME_ELIGIBILITIES, REVIEWER_APPROVAL_ROLE, REVIEWER_ATTEMPT_SPEC, REVIEWER_RESULTS, REVIEW_GREEN_1, REVIEW_GREEN_2, REVIEW_LENS_CATEGORIES, REVIEW_VERDICTS, RUN_ACTIVITY_STATES, RUN_ACTOR_ROLES, RUN_CANDIDATE_TREE_SHA, RUN_COMMAND_OUTCOMES, RUN_ENDED_RESULTS, RUN_EVENT_KINDS, RUN_EVENT_KINDS_V1, RUN_EVENT_PAYLOAD_GRAMMAR_SPEC, RUN_EVENT_SPEC, RUN_EVENT_SPEC_V2, RUN_FREE_TEXT_MEMBERS, RUN_GATE_REPORTED_OUTCOMES, RUN_JOURNAL_REQUIRED_ENTRIES, RUN_JOURNAL_ROUND_BINDINGS, RUN_JOURNAL_STATUSES, RUN_JOURNAL_VIOLATIONS, RUN_PROVIDER_ID, RUN_ROOT_LEAF, RUN_ROOT_NAMESPACE, RUN_ROOT_REFUSAL_REASONS, RUN_STORE_DIRECTORY, RUN_STORE_ID, RUN_TICKET, ReviewInputError, SCOPED_DELIVERY_CONTRACT_SPEC, SECRET_PATTERNS, SECURITY_BLOCKED_MIGRATION_ACTION, SENSITIVE_APPROVAL_ASSERTION_SPEC, SENSITIVE_MAINTENANCE_ACTIONS, SENSOR_OUTCOMES, SENSOR_RESULT_SPEC, SHADOW_MILESTONE_GATE_RECORD_SPEC, SHADOW_MILESTONE_GATE_RECORD_SPEC_SUFFIX, SIMULATED_MESSAGE_OPTION_NAMES, SPINE_GIT_OID, SPINE_ID, SPINE_INSTANT, SPINE_SHA256, SUBMISSION_CANDIDATE_FIELDS, SUBSTRATE_BLOCKER_CODES, SUPPORTED_CONTRACT_VERSIONS, SUPPORTED_ENVELOPE_SPECS, SUPPORTED_PAYLOAD_SPECS, SUPPORTED_PLATFORMS, SUSPENDED_DELIVERY_STATES, TERMINAL_DELIVERY_STATES, TERMINATION_PROVENANCE_KINDS, TERMINATION_PROVENANCE_OPERATION, TOOL_DENIAL_CODES, TRACKER_ABSENCE_FALLBACKS, TRACKER_POSTURES, UNBOUND_COORDINATION_PORT, UNBOUND_EXTERNAL_ACTION_PORT, UNKNOWN_CONTEXT_REASONS, V1_ATTESTATION_LEVEL, VALIDATOR_EMITTED_CODES, WAIVER_ACTIONS, WAIVER_APPROVAL_ORIGIN_PREFIX, WAIVER_SCOPES, WORKFLOW_CHECKPOINT_BINDINGS, WORKFLOW_GRAPH_ENTRY, WORKSPACE_DISPOSITIONS, actionChainDigest, adaptClaudeCodeReviewResult, admitCoordinationMessage, applySecretDiscipline, assertSha256Hex, assertionClassOf, assertionLaneAvailability, assertionProviderConfigPathFor, authorizeFinishLineAction, bindingOf, buildCompositionManifest, buildDeliveryRecord, candidateTreeEvidenceReader, candidateTreeSourceReader, canonicalBytes, canonicalize, captureCheckBindings, captureCheckOutputSnapshots, captureCheckOutputs, captureGitCandidate, capturePortableEvidenceContext, capturePortableVerificationInputs, captureRunArtifact, captureScopedCheckInputs, checkActionAuthorization, checkBoundPolicy, checkClaimAuthorized, checkContractWithinPolicy, checkDeployPreconditions, checkFacadeSurfaceInvariants, checkMergeReadyAgainstOutcome, checkMutationLane, checkNoDowngrade, checkOutcomeCoversContract, checkPositiveCriterion, checkReviewFloor, claimContradictsLocalHistory, classifyActionOutcome, classifyCandidateDrift, classifyCandidatePath, classifyEventKind, classifyExecutionContext, claudeCodeBinding, codexAppServerBinding, codexEnabledToolsFor, codexEscalationTokens, codexHookDecision, codexHookTurn, codexHostTool, codexNamesDisabledSurface, codexPermissionProfileId, codexPermissionProfileOf, codexSubagentPosture, codexToolIsLocallyEnforceable, codexWrittenPaths, collectLiveProviderResults, compareSubmissionCandidate, compareUtf16CodeUnits, compileDisposableCompiledPolicy, compileDisposablePolicy, compileRepositoryPolicy, compiledAdopterPolicyBindingDigest, composeBlockerInventory, composeClaudeCodeSession, composeCodexAppServerThread, composeManagedStatus, composeOutcomeVerification, compositionManifestBytes, computeCheckWiringFingerprint, computeDeliverableIdentity, computePreparationFingerprint, computeRecordId, confirmationClassOf, conflictBlockerEpochOf, consumeProviderRailMessages, createArtifactsPort, createBlocker, createCandidateCapture, createCodexAppServerConformancePort, createCoordinationSimulator, createExecPort, createFakeHostConformancePort, createIntakeJournalStore, createInternalErrorBlocker, createJournalStore, createMaintenanceJournalStore, createManagedDeliveryFacade, createOsNativeAssertionSource, createProviderReviewHandoff, createQualificationFixtureAssertionSource, createRunStore, decideCodexHookInvocation, decideFinishLine, defaultRunRootBase, defineHarnessConfig, deleteDelivery, deliveryRecordBytes, deliveryRecordPathFor, deriveDeliveryRecordPath, deriveTelemetry, describeRunEventPayload, digestCanonical, digestDeliverableEntries, digestsEqual, discoverRecords, discriminateInstall, effectiveDeliveryAuthority, effectiveHostedChecksPolicy, emittableFindingCodes, enforceAllowedResolution, entryHref, evaluateActionApproval, evaluateCandidateActivation, evaluateCanonicalRecheck, evaluateCodexEscalation, evaluateConfirmationEcho, evaluateGate, evaluateHostAdmission, evaluateMigrationConsumption, evaluatePreparationReceipt, evaluateRunJournal, evaluateToolInvocation, evaluateWaiverConsumption, explainRunJournal, exportDelivery, facadeOperation, faithfullyAppliedCodexConfiguration, garbageCollectGenerations, generationDigestOf, gitNamespaceClearedEnvironment, gradeHostActivity, grantDigest, identityDefinitionOf, inspectInstallation, installComposition, invalidatePreparationReceipt, invokeExternalActionOnce, invokeProviderRail, invokedDirectly, isDeliveryOwnedTreeEntry, isDeliveryOwnedTreePath, isDeliveryRecordRunSpan, isDeliveryTransitionValid, isEnvSignalPresent, isHostedCheckExemption, isHostedCheckInstant, isHostedChecksPolicy, isInsideResolved, isIntakeTransitionValid, isManifestRejectionCode, isObligationActive, isRecordFreshForCandidate, isRecordNeutralPath, isReviewNeutralPath, isRunEventKind, isRunInstant, isSafeRelativePath, isScopedCheckDefinition, isSha256Hex, journalIntentPayload, journalResultPayload, listArchiveEntries, livePreflightProbes, loadAssertionProviderConfig, loadBundledWorkflowGraph, loadPinnedGeneration, localDigestTrustPredicate, localFactEpochOf, maintainTrustState, manifestDigest, matchesNeutralSet, matchesPathMatcher, materializeProjection, mintGrantAttestation, needsCommittedSymlinkTarget, neutralizeForDisplay, observeAuthorityEpoch, openProviderRailProcess, operationsOnSurface, packComposition, parseCandidateNumstat, parseCandidateTreeListing, parseCodexHookInput, parseDeliveryRecord, parseProjectionConsumptionObservation, parseProviderReviewResult, parseReviewOutcome, parseTreeEntries, parseTrustState, planExternalAction, portableArtifactContents, projectReviewActivation, projectRunActivities, projectShippedPersonas, projectionConsumptionObservationFile, publishPreparationReceipt, publishRecord, qualifyReviewAttempts, readArchiveEntry, readCompiledRepositoryPolicy, readConsumptionMarker, readRunArtifact, readWorkflowRelease, receiptFileName, receiptPathFor, reconcileBeforeRetry, reconcileRemoteClaim, recordFileName, recordIdentity, recoverInterruptedMaintenance, redactSecretText, reduceDeliveryJournal, reduceIntakeJournal, reduceToProviderId, registrationBinding, remediationFor, renderBlockers, renderCodexHookDecision, repairInstallation, replayLedgerOf, repositoryEvidenceReader, resolveActiveGeneration, resolveCommonDirectoryNamespace, resolveObservationLifetimeSeconds, resolveReceiptStorage, resolveRecordStorage, resolveReviewCharters, resolveRunStoreLocation, retainedCheckOutput, revalidateBeforeInvoke, reviewerLists, revokePreparationAttempt, rollbackComposition, runActivityTransitionError, runAdmission, runGitCommand, runGitDirect, runHostIntegrationConformance, runJournalCarries, runPrimaryTicket, sanitizedDetail, scopedCheckIdentity, selectDeliveryRecordForIdentity, selectScopedCheckAttempt, sensitiveGroupsFor, serializeBlockers, sha256Hex, submitManifest, trackerPostureOf, trustStorePathFor, updateComposition, validateAcceptedContract, validateAdapterCapability, validateAdapterSet, validateAuthorityRevocation, validateCapabilityDescriptor, validateCompositionManifest, validateCompositionPin, validateCoordinationMessage, validateExecutionGrant, validateExternalActionResult, validateFinishLineResult, validateGrantAttestation, validateHarnessConfig, validateInvocationFence, validateJournalEntry, validateManifest, validateOperatorConfirmation, validateOutcomeVerification, validatePolicySnapshot, validateProductTrustState, validateRepositoryPolicyDocument, validateReviewGreenClaim, validateReviewedContext, validateReviewerAttempt, validateRunEvent, validateRunEventInput, validateSensitiveApprovalAssertion, validateSensorResult, verifyAppliedCodexThreadConfiguration, verifyCompiledPolicy, verifyDeliveryRecord, verifyGenerationClosure, verifyPortableEvidence, verifyProjection, withDeliverableIdentity, withProcessLock, workflowStageBindingFor, writeAssertionProviderConfig };
export type { AcceptedContract, ActionAdapterBinding, ActionApprovalContext, ActionApprovalVerdict, ActionChainLink, ActionClassification, ActionEvidence, ActivationKind, AdapterCapability, AdditionalReviewLens, AdmissionDecision, AdmissionDenial, AdmissionDenialCode, AdmissionExpectation, AdmissionInput, AdmissionOptions, AdmissionResult, AdmittedInvocation, AgentExecutionContext, ArtifactObservation, ArtifactObservationStatus, ArtifactPresence, ArtifactsPort, ArtifactsPortOptions, AssertionAvailabilityProbe, AssertionClass, AssertionEvaluation, AssertionEvaluationRequest, AssertionProviderConfig, AssertionSource, AssertionSourceAvailability, AssertionSourcePort, AssertionSourceView, AttestationLevel, AuthorityGrantView, AuthorityRevocation, AvailablePersona, BaseMovementPolicy, BlockedResolution, Blocker, BlockerInput, BlockerInventoryEntry, BlockerSource, BlockerSourceKind, BoundActionIntent, BuildCompositionManifestInput, BuildDeliveryRecordInput, BuildDeliveryRecordResult, CandidateActivationOptions, CandidateBase, CandidateBinding, CandidateCapture, CandidateCaptureCode, CandidateCaptureOptions, CandidateCommandResult, CandidateCommandRunner, CandidateComparison, CandidateDeliverable, CandidateDiffEntry, CandidateDriftClass, CandidateMode, CandidatePathClass, CandidateStatusEntry, CandidateTreeEntry, CandidateVcs, CanonicalErrorCode, CaptureCandidate, CapturedCandidate, CheckActionAuthorizationInput, CheckBinding, CheckBindingOptions, CheckpointAdmissionExpectation, CheckpointOverride, CiExecutionContext, CiPolicy, ClaimAuthorityView, ClaimDisposition, ClaimReconciliation, ClassifyExecutionContextInput, ClaudeCodeReviewAdaptation, CodexAppServerConformancePortInput, CodexAppliedMismatch, CodexAppliedMismatchCode, CodexAppliedThreadConfiguration, CodexDisabledFeatureKey, CodexEscalationRefusalCode, CodexEscalationRequest, CodexHookInput, CodexHostTool, CodexPermissionProfile, CodexThreadStartRequest, CodexUnenforceableToolSource, CollectLiveProviderInput, CommandArguments, CompareCheck, CompileDisposablePolicyInput, CompileRepositoryPolicyInput, CompileRepositoryPolicyResult, CompiledAdopterPolicyBinding, CompiledCheckpointGrant, CompiledPolicy, ComposeCodexAppServerThreadResult, ComposeHostSessionInput, ComposeHostSessionResult, ComposedHostSession, CompositionInventoryEntry, CompositionProfile, ComputeIdentity, ConfigFindingCode, ConfirmationClass, ConfirmationDenial, ConfirmationDenialCode, ConfirmationEchoAttempt, ConfirmationEchoDecision, ConsumeProviderRailOptions, ConsumedWaiver, ConsumptionGateRecordBlockerCode, ConsumptionMarker, ControlPlaneClaim, CoordinationAdmission, CoordinationAdmissionView, CoordinationAuthentication, CoordinationDispatch, CoordinationMessage, CoordinationMessageKind, CoordinationPort, CoordinationRefusal, CoordinationRefusalCode, CoordinationReplayLedger, CoordinationSimulator, CoordinationSimulatorOptions, CreateFacadeInput, DeclaredArtifact, DelegatedResolution, DeleteDeliveryResult, DeliverableIdentityDefinition, DeliverableIdentityOptions, DeliverableIdentityRequest, DeliverableTreeEntry, DeliveryEvidenceManifest, DeliveryJournalState, DeliveryRecord, DeliveryRecordAttestation, DeliveryRecordCheck, DeliveryRecordClaim, DeliveryRecordDriftClass, DeliveryRecordFile, DeliveryRecordHostedChecks, DeliveryRecordRunSpan, DeliveryRecordVerification, DeliveryState, DeliveryTransitionRow, DeniedInvocation, DeployPreconditionInput, DigestErrorCode, EffectiveDeliveryAuthorityInput, EligibleCheck, EmitProjectionConsumptionResult, EnvSnapshot, EnvironmentRequirement, EvaluateGateInput, EventClassification, EventKindEntry, EvidenceRecord, EvidenceRecordIdentity, EvidenceResolution, ExecInvocation, ExecOutcome, ExecPort, ExecutionContext, ExecutionContextKind, ExportDeliveryResult, ExternalAction, ExternalActionIntent, ExternalActionInvocation, ExternalActionPort, ExternalVerification, FacadeCapabilityClass, FacadeFailure, FacadeFenceRule, FacadeJournalRule, FacadeOperation, FacadeSurface, FacadeSurfaceFinding, FacadeSurfaceRule, FinishLineDecision, FinishLineInput, FinishLineRefusal, FinishLineResult, FreshnessKind, GarbageCollectInput, GateDecision, GateStructuralFindingCode, GitRunner, GradeHostActivityInput, GrantProfile, HarnessConfig, HarnessConfigInput, HarnessConfigValidation, HostActivity, HostActivityState, HostAdmissionScenario, HostBindingBlocker, HostBindingBlockerCode, HostBindingRefusalDetail, HostConformanceCase, HostConformanceResult, HostIntegrationPort, HostInterceptionScenario, HostObservationStamp, HostSessionGrant, HostedCheckExemption, HostedCheckExemptionScope, HostedChecksPolicy, HumanExecutionContext, IdentityFindingCode, IgnoredStoreEntry, InspectInstallationInput, InspectedGeneration, InstallCompositionInput, InstallCompositionResult, InstallDiscrimination, InstallReceipt, InstallationPresence, IntakeAdmissionExpectation, IntakeJournalState, IntakeJournalStore, IntakeState, InvocationOutcome, Journal, JournalAppendResult, JournalReadResult, JournalStore, LaneAvailability, ListedDelivery, ListedTreeEntry, LiveProviderCollection, LiveProviderFinding, LiveProviderResult, LoadPinnedGenerationResult, LocalHistoryView, MaintainTrustStateInput, MaintenanceJournalStore, ManagedCheckpoint, ManagedDeliveryFacade, ManagedDeliveryStatus, ManagedHostBinding, ManagedInstallation, ManagedStatusInput, ManifestArtifact, ManifestAttestation, ManifestBase, ManifestCandidate, ManifestClaim, ManifestDeliverable, ManifestProvider, ManifestRejection, ManifestRejectionCode, ManifestRejectionCodeEntry, ManifestRuleId, ManifestValidation, ManifestValidationContext, MigrationConsumptionContext, MigrationConsumptionVerdict, MigrationPath, MigrationRefusal, MirrorRecordView, MirroredMessageView, MutationLaneResult, MutationVerification, NamespaceGitLaunch, NamespaceGitRunner, NamespaceResolution, NamespaceResolutionInput, NativeReviewConclusion, NeutralMatcher, NoDowngradeDecision, NonEmptyTuple, NormalizedAdmission, NormalizedInterception, NormalizedTeardown, NormalizedTermination, NotApplicableResolution, ObligationActivation, ObligationFinding, ObligationPolicy, ObligationResolution, ObserveAuthorityEpochResult, ObservedActionResult, ObservedActivityEntry, OpenProviderRailProcessInput, OtherInstallationArtifact, OutcomeCriterion, OutcomeVerification, PackCompositionInput, PackCompositionResult, ParseDeliveryRecordResult, ParseTrustStateResult, PathClassification, PathMatcher, PathMatcherKind, PlanExternalActionInput, PlanExternalActionVerdict, PolicyCapabilityKind, PolicyGrantView, PolicyRejection, PolicySnapshot, PolicyVerdict, PortableEvidence, PortableEvidenceContext, PositiveCriterionVerdict, PreflightProbes, PreparationCandidate, PreparationCommand, PreparationEvaluation, PreparationFailureClass, PreparationInput, PreparationOptions, PreparationReceipt, ProductTrustPort, ProductTrustState, ProductTrustView, ProjectShippedPersonasResult, ProjectionConsumptionObservation, ProjectionConsumptionRecord, ProjectionConsumptionUnobserved, ProviderPolicy, ProviderRailAttemptInput, ProviderRailAttemptOptions, ProviderRailBlocker, ProviderRailCancel, ProviderRailConsumption, ProviderRailEvent, ProviderRailEvidence, ProviderRailInvocationResult, ProviderRailMessage, ProviderRailNegotiate, ProviderRailNegotiation, ProviderRailProgress, ProviderRailRequest, ProviderRailSession, ProviderRailTerminal, ProviderRegistration, ProviderReviewCandidate, ProviderReviewCapability, ProviderReviewFinding, ProviderReviewHandoff, ProviderReviewHandoffReviewer, ProviderReviewParseResult, ProviderReviewResult, ProviderReviewTerminalState, ProviderReviewVerdict, PublishOptions, PublishRecordInput, PublishStatus, PublishedPreparationReceipt, PublishedRecord, QualifiedAttempts, QuarantinedRecord, ReadConsumptionMarkerResult, RecheckConsumption, RecheckFailure, RecheckResult, RecheckValues, RecheckedValue, RecomputedIdentity, ReconciliationDisposition, ReconciliationFinding, RecordCandidateBinding, RecordDiscovery, RecordIdentity, RecordQuarantineReason, RecordResolution, RecordSelector, RecordStorageOptions, RecordedHostedCheckExemption, RecordedRegistrationBinding, RecordedReviewAttempt, RecordedSensorResult, ReduceDeliveryResult, ReduceIntakeResult, RegistrationBindingInput, RegistrationBindingResult, RegistrationBindingView, RegistrationMismatch, RejectionEmitter, Remediation, RemediationCatalog, RenderOptions, RenderedConfirmationChallenge, RepairInstallationInput, RepositoryPolicyDocument, ResolutionKind, ResolutionOutcome, ResolveActiveGenerationResult, ResolvedCharter, ResolvedPersonaSource, RetentionContext, RetentionFailure, RetrySafety, RevalidationObservation, ReviewActivationProjection, ReviewGreenClaimInput, ReviewInputReader, ReviewOutcome, ReviewRejection, ReviewVerdict, ReviewerOutcome, ReviewerResult, RollbackCompositionInput, RunActivityObservation, RunActivityProjection, RunActivityState, RunActorRole, RunAllocateResult, RunAppendResult, RunArtifactMetadata, RunArtifactResult, RunAttemptObservation, RunCommandOutcome, RunCurrentResult, RunDiscardResult, RunEvent, RunEventActor, RunEventInput, RunEventKind, RunEventPayloadGrammar, RunEventPayloadMemberGrammar, RunEventRepo, RunEventValueGrammar, RunEventValueType, RunEventVersion, RunHistoryEntry, RunJournalDiagnostics, RunJournalEvaluation, RunJournalExplanation, RunJournalRequiredEntry, RunJournalRoundBinding, RunJournalRow, RunJournalStatus, RunJournalViolation, RunMatch, RunPreparationObservation, RunReadResult, RunReferencedObservation, RunRoot, RunRootRefusalReason, RunRootRequest, RunRootResolution, RunSetCurrentResult, RunStore, RunStoreLocation, RunStoreRejection, RunStoreRejectionCode, RunWaitObservation, SatisfiedEvidenceResolution, SatisfiedLiveFactResolution, ScopedCheckAttempt, ScopedCheckDefinition, ScopedCheckPlan, ScopedExecutionDefinition, ScopedExecutionProfile, ScopedInputCapture, ScopedInputCapturePorts, ScopedRuntimeObservation, SecretDisciplineResult, SecretPattern, SensitiveLaneInput, SensitiveMaintenanceAction, SensitivePathGroup, SerializedBlocker, SerializedBlockers, SerializedRemediation, ShippedPersonaRejection, ShippedPersonaRejectionCode, SimulatedExchange, SimulatedMessageOptions, SpineRejection, SpineRejectionCode, SpineVerdict, SubmissionInput, SubmissionOptions, SubmissionOutcome, SubmissionRecord, SubstrateBlocker, SubstrateBlockerCode, SubstrateFailure, SubstrateRejection, SubstrateRejectionCode, SubstrateVerdict, ToolDenial, ToolDenialCode, ToolInvocationDecision, ToolInvocationRequest, TrackerPosture, TrustCheckInput, TrustDecision, UnknownContextReason, UnknownExecutionContext, UnreadableRecordInput, UpdateCompositionInput, UpdateCompositionResult, ValidBlockerCode, ValueCheck, VerificationBase, VerifiedClosure, VerifyAppliedCodexThreadConfiguration, VerifyDeliveryRecordOptions, VerifyProjectionResult, WaivedResolution, WaiverAction, WaiverApproval, WaiverConsumptionContext, WaiverConsumptionVerdict, WaiverPrompt, WaiverPromptOutcome, WaiverProposal, WaiverRecordIdentity, WaiverRefusal, WaiverResolution, WaiverScope, WorkflowCheckpointBinding, WorkflowGraph, WorkflowStage, WorkspaceDisposition, WorkspaceStorage, WriteFileOptions };
