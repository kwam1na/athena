...
var ScopedChecks = class _ScopedChecks {
  ...
  static async create(context, candidate) {
    ...
    const storage = await resolveRecordStorage3(context.rootDir, { storageNamespace: context.config.storageNamespace, leaf: "scoped-attempts" });
  ...
  async execute(provider, obligationIds, deferSubmission = false) {
    ...
    try {
      ...
      const redact = (s) => secrets.reduce((text2, secret) => text2.split(secret).join("[REDACTED]"), s);
...
${result.stderr}`.slice(-4e3)}`);
  ...
  if (snapshots === void 0 || digestCanonical5(snapshots.map(({ path: path22, sha256 }) => ({ path: path22, sha256 }))) !== binding.outputsDigest) return fail("check_output_missing", "Declared outputs changed before evidence retention.");
...
import {
  ...
  resolveRecordStorage as resolveRecordStorage4,
...
async function applyDeliveryRecordRetention(input) {
  ...
  const storage = await resolveRecordStorage4(input.rootDir, { storageNamespace: input.storageNamespace, leaf: RETENTION_LEAF });
...
async function readScopedCheckObservations(input) {
  ...
  for (const provider of config.providers.filter((provider2) => provider2.check?.scope !== void 0)) {
    const store = new AttemptStore(path21.join(storage.storageDir, digestCanonical9({ gate: config.gateId, provider: provider.id })));
