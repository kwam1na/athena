...
function redactSecrets(value) {
  return value.replace(/-----BEGIN (?:[A-Z ]+ )?PRIVATE KEY-----[\s\S]*?-----END (?:[A-Z ]+ )?PRIVATE KEY-----/gi, "[REDACTED PRIVATE KEY]").replace(/-----BEGIN (?:[A-Z ]+ )?PRIVATE KEY-----[\s\S]*/gi, "[REDACTED PRIVATE KEY]").replace(
    ...
    "[REDACTED]"
  ).replace(/(?:AKIA|ASIA)[0-9A-Z]{16}/g, "[REDACTED]").replace(/eyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]+/g, "[REDACTED]").replace(/(authorization\s*:\s*bearer\s+)[^\s]+/gi, "$1[REDACTED]").replace(
    /\b(bearer|token)[ \t\r\n]+([A-Za-z0-9._~+/-]{16,}=*)/gi,
...
function readShape(findings2, input) {
  ...
  if (preparationWiringPaths !== void 0) checkDuplicateIds(findings2, "preparationWiringPaths", preparationWiringPaths);
...
function checkInvariants(findings2, config) {
  ...
  for (const member2 of ["acceptedEnvelopeSpecs", "identityVersions", "agentEnvSignals"]) {
    checkDuplicateIds(findings2, member2, config[member2]);
  }
  for (const [index2, provider2] of config.providers.entries()) {
    const member2 = `providers[${index2}].findingCodes`;
    checkDuplicateIds(findings2, member2, provider2.findingCodes);
    for (const code of provider2.findingCodes) {
      if (GATE_STRUCTURAL_FINDING_CODES.includes(code)) {
        findings2.add("config_code_collision", member2, `${JSON.stringify(code)} is reserved by the structural finding registry`);
      }
    }
  }
  ...
  for (const [index2, obligation] of config.obligations.entries()) {
    ...
    checkDuplicateIds(findings2, `${at}.waivableCodes`, obligation.waivableCodes);
    checkDuplicateIds(findings2, `${at}.nonWaivableCodes`, obligation.nonWaivableCodes);
...
async function observe(run, rootDir, config) {
  ...
  if (indexTree.exitCode !== 0) {
    if (/index\.lock['"]?: File exists/i.test(indexTree.stderr)) {
      return blocked2(config, "candidate_repository_unreadable", "Git could not acquire the index lock", indexTree.stderr, [
        {
          id: "retry-index-lock",
          kind: "manual_action",
...
function digestDeliverableEntries(entries, definition) {
  ...
  const deliverable = entries.filter((entry3) => !matchesNeutralSet(definition.reviewNeutral, entry3.path) || definition.identityToken === "validation-tree/v1" && entry3.mode !== "100644" && entry3.mode !== "100755").sort((left, right) => compareUtf16CodeUnits(left.path, right.path));
...
function createArtifactsPort(options = {}) {
  let resolvedBase;
  const base = () => resolvedBase ??= options.runRootBase === void 0 ? defaultRunRootBase() : realpath2(options.runRootBase);
...
var malformed = (collector, at, message) => {
...
function isHostedCheckInstant(value) {
  ...
  if (year === void 0 || month === void 0 || day === void 0 || hour === void 0 || minute === void 0 || second === void 0) return false;
  if (month < 1 || month > 12 || hour > 23 || minute > 59 || second > 59) return false;
  const leap = year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0);
  const days = [31, leap ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  return day >= 1 && day <= (days[month - 1] ?? 0);
}
var hostedCheckInstant = (value, at, collector) => {
...
var HOSTED_CHECK_SCOPE_RULES = [
  { name: "repositoryId", check: spineId },
  { name: "baseRef", check: boundedBaseRef }
];
var HOSTED_CHECK_EXEMPTION_RULES = [
...
var HOSTED_CHECK_RULES = [
  { name: "required", check: literal(true) },
  { name: "exemptions", check: closedArray(HOSTED_CHECK_EXEMPTION_RULES) }
];
var hasExactKeys = (value, keys) => Object.keys(value).sort().join("\0") === [...keys].sort().join("\0");
function isHostedCheckExemption(value) {
  ...
  const scoped = scope;
  return hasExactKeys(scoped, ["repositoryId", "baseRef"]) && typeof scoped["repositoryId"] === "string" && SPINE_ID.test(scoped["repositoryId"]) && typeof scoped["baseRef"] === "string" && scoped["baseRef"].length > 0 && scoped["baseRef"].length <= 256 && !/[\u0000\r\n]/.test(scoped["baseRef"]) && typeof exemption["reason"] === "string" && exemption["reason"].length > 0 && exemption["reason"].length <= MAX_FREE_TEXT && typeof exemption["grantedBy"] === "string" && exemption["grantedBy"].trim().length > 0 && exemption["grantedBy"].length <= 256 && isHostedCheckInstant(exemption["until"]);
}
function isHostedChecksPolicy(value) {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return false;
  const policy = value;
  return hasExactKeys(policy, ["required", "exemptions"]) && policy["required"] === true && Array.isArray(policy["exemptions"]) && policy["exemptions"].every(isHostedCheckExemption);
}
...
function compileRepositoryPolicy(input) {
  ...
  const hostedChecks = document.hostedChecks ?? { required: true, exemptions: [] };
  const hostedScopes = /* @__PURE__ */ new Set();
  hostedChecks.exemptions.forEach((exemption, index2) => {
    if (exemption.scope.repositoryId !== document.repositoryId) {
      collector.emit(
        "hosted_check_exemption_repository_mismatch",
        `/document/hostedChecks/exemptions/${index2}/scope/repositoryId`,
        `hosted-check exemption scope ${exemption.scope.repositoryId} does not name policy repository ${document.repositoryId}`
      );
    }
    const key = `${exemption.scope.repositoryId}\0${exemption.scope.baseRef}`;
    if (hostedScopes.has(key)) {
      collector.emit(
        "duplicate_hosted_check_exemption_scope",
        `/document/hostedChecks/exemptions/${index2}/scope`,
...
function verifyCompiledPolicy(value) {
  ...
  if (record2["hostedChecks"] !== void 0 && !isHostedChecksPolicy(record2["hostedChecks"])) {
...
function effectiveHostedChecksPolicy(policy) {
...
async function readCompiledRepositoryPolicy(read) {
  ...
  const compiled = isRecord4(wrapper) ? wrapper["compiled"] : void 0;
  if (!isRecord4(compiled) || compiled["hostedChecks"] === void 0) return null;
  const verdict = verifyCompiledPolicy(compiled);
  if (!verdict.ok) {
    throw new ReviewInputError(`the compiled policy snapshot is invalid: ${verdict.rejections.map((entry3) => `${entry3.pointer} [${entry3.code}] ${entry3.message}`).join("; ")}`);
  }
  return compiled;
}
...
async function submitManifest(input, options) {
  ...
  if (captured !== null) {
    const preparation2 = await evaluatePreparationReceipt(
      input.rootDir,
    ...
    if (!preparation2.prepared) return blockedOutcome(preparation2.blockers);
    preparationFingerprint = preparation2.receipt.preparationFingerprint;
  }
  const rejections = [];
  ...
  if (runRoot !== null) {
    ...
    for (const entry3 of declaredArtifacts(manifest)) {
      const observation = await artifacts.observeArtifact(runRoot, entry3.path);
...
async function admit(input, options) {
  ...
  const preparation2 = await evaluateReceipt(input.rootDir, input.config, candidate2);
  if (!preparation2.prepared) {
    return blocked3({ ...NOT_OFFERED, candidate: candidate2, context: input.context, blockers: preparation2.blockers });
  }
  ...
  if (!recapture.ok || !sameCandidate(candidate2, recapture.candidate) || !currentPreparation?.prepared || currentPreparation.receipt.preparationFingerprint !== preparation2.receipt.preparationFingerprint) {
    return blocked3({
...
function activeHostedCheckExemption(compiledPolicy, binding2, observedAt) {
  ...
  const scoped = policy.exemptions.find((entry3) => entry3.scope.repositoryId === compiledPolicy.snapshot.repositoryId && entry3.scope.baseRef === binding2.baseRef);
  if (scoped === void 0) return { ok: true };
...
function buildDeliveryRecord(input) {
  ...
  let hostedChecks;
  if (input.compiledPolicy !== void 0) {
    ...
    hostedChecks = { required: true, ...active.exemption === void 0 ? {} : { exemption: active.exemption } };
  }
  const record2 = {
    ...
    context,
    ...hostedChecks === void 0 ? {} : { hostedChecks }
  };
...
function isRecordedHostedCheckExemption(value) {
  if (!isRecord7(value)) return false;
  const { policyDigest, ...exemption } = value;
  return Object.keys(value).sort().join("\0") === ["grantedBy", "policyDigest", "reason", "scope", "until"].join("\0") && typeof policyDigest === "string" && /^[a-f0-9]{64}$/.test(policyDigest) && isHostedCheckExemption(exemption);
}
function isDeliveryRecordHostedChecks(value) {
  if (!isRecord7(value) || value["required"] !== true) return false;
  const keys = Object.keys(value).sort().join("\0");
  return (keys === "required" || keys === "exemption\0required") && (value["exemption"] === void 0 || isRecordedHostedCheckExemption(value["exemption"]));
}
function parseDeliveryRecord(text4) {
  if (Buffer.byteLength(text4) > MAX_PORTABLE_RECORD_BYTES) return malformed2("the record exceeds the portable size limit");
  let parsed;
  try {
    parsed = JSON.parse(text4);
  } catch (error) {
    return malformed2(`not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
  if (!isRecord7(parsed)) return malformed2("the record is not a JSON object");
  if (parsed["version"] !== DELIVERY_RECORD_VERSION && parsed["version"] !== "delivery-record/1") {
    return malformed2(`unsupported version token ${JSON.stringify(parsed["version"])}; expected ${DELIVERY_RECORD_VERSION}`);
  }
  if (!isNonEmptyString3(parsed["gateId"])) return malformed2("missing gateId");
  if (!isNonEmptyString3(parsed["identityToken"])) return malformed2("missing identityToken");
  if (!isNonEmptyString3(parsed["workspaceId"])) return malformed2("missing workspaceId");
  const binding2 = parsed["candidateBinding"];
  if (!isRecord7(binding2)) return malformed2("missing candidateBinding");
  for (const field of BINDING_FIELDS) {
    if (!isNonEmptyString3(binding2[field])) return malformed2(`candidateBinding is missing ${field}`);
  }
  if (parsed["identityToken"] !== binding2["identityToken"]) {
    return malformed2("identityToken disagrees with candidateBinding.identityToken");
  }
  const claims = parsed["claims"];
  if (!Array.isArray(claims)) return malformed2("claims must be an array");
  const obligationIds = /* @__PURE__ */ new Set();
  for (const claim of claims) {
    if (!isRecord7(claim) || !isNonEmptyString3(claim["obligationId"]) || !isNonEmptyString3(claim["outcome"])) {
      return malformed2("a claim is missing its obligation id or outcome");
    }
    if (obligationIds.has(claim["obligationId"])) return malformed2("claims repeat an obligation id");
    ...
    if (claim["outcome"] === "waived" && (!isAttributedWaiver(claim["waiver"]) || claim["scope"] !== claim["waiver"].scope)) {
      return malformed2("a waived claim requires attributed, scoped approval bound to its policy and candidate");
    }
  }
  const attestation = parsed["attestation"];
  if (!isRecord7(attestation) || !isNonEmptyString3(attestation["level"])) return malformed2("missing attestation.level");
  const manifestDigest2 = parsed["manifestDigest"];
  if (manifestDigest2 !== null && typeof manifestDigest2 !== "string") return malformed2("manifestDigest must be a string or null");
  if (parsed["hostedChecks"] !== void 0 && !isDeliveryRecordHostedChecks(parsed["hostedChecks"])) {
    return malformed2("hostedChecks must keep checks required and carry only a complete attributed exemption");
  }
...
function verifyDeliveryRecord(config, record2, recomputedIdentity, base, options = {}) {
  ...
  let hostedChecks = { status: "required" };
  ...
  const recordedExemption = isDeliveryRecordHostedChecks(record2.hostedChecks) ? record2.hostedChecks.exemption : void 0;
  if (record2.hostedChecks !== void 0 && !isDeliveryRecordHostedChecks(record2.hostedChecks)) {
  ...
  } else if (options.compiledPolicy === void 0) {
    if (recordedExemption !== void 0) {
    ...
    const active = activeHostedCheckExemption(options.compiledPolicy, binding2, options.observedAt);
    if (!active.ok) {
      blockers.push(active.blocker);
    } else if (active.exemption === void 0) {
      if (recordedExemption !== void 0) {
        const declared = effectiveHostedChecksPolicy(options.compiledPolicy).exemptions.find((entry3) => entry3.scope.repositoryId === options.compiledPolicy.snapshot.repositoryId && entry3.scope.baseRef === binding2.baseRef);
        const exactPolicyDeclaration = declared !== void 0 && recordedExemption.policyDigest === options.compiledPolicy.compiledDigest && digestCanonical(declared) === digestCanonical((({ policyDigest: _policyDigest, ...rest }) => rest)(recordedExemption));
        blockers.push(drBlocker(
          exactPolicyDeclaration && isHostedCheckInstant(options.observedAt) && options.observedAt >= recordedExemption.until ? "hosted_check_exemption_expired" : "hosted_check_exemption_unrecognized",
          exactPolicyDeclaration ? "The recorded hosted-check exemption has expired." : "The recorded hosted-check exemption is not an exact current owner declaration for this repository and base ref."
        ));
      }
    } else if (recordedExemption === void 0) {
  ...
  for (const driftClass of baseDrift) {
    if (policy === "allow") {
      ...
      blockers.push(drBlocker(
        ...
        `recorded base ${binding2.baseRef} at ${binding2.baseTipSha} (merge base ${binding2.mergeBaseSha}); observed base ${base.ref} at ${base.tipSha} (merge base ${base.mergeBaseSha})`,
        {
          id: "reconcile-base-movement",
          kind: "manual_action",
          summary: "Confirm whether the observed base includes this delivery's own confirmed merge. If so, retain the pre-merge verification and merge evidence; no new delivery loop is required. Otherwise, refresh the candidate, preparation, review evidence, gate, and record within the same run while it remains open. If that run already ended, link the retry with predecessorRunId. This stale-record result does not authorize a merge."
        }
      ));
    }
  }
...
var CHECK_VALUES = /* @__PURE__ */ new WeakMap();
function checkRunClosed(value, at, rules, collector) {
  const accepted = rules.map((rule) => rule.name).join(", ");
  return checkClosed2(value, at, rules, {
    emit(code, pointer2, message) {
      const relative = pointer2.startsWith(`${at}/`) ? pointer2.slice(at.length + 1) : "";
      const directMember = relative !== "" && !relative.includes("/");
      collector.emit(
        code,
        pointer2,
        directMember && (code === "unknown_member" || code === "missing_member") ? `${message}; accepted members: ${accepted}` : message
      );
    },
...
var oneOf2 = (values) => {
  const check = (value, at, collector) => {
    if (typeof value !== "string" || !values.includes(value)) {
      const encoded = at.split("/").at(-1) ?? "member";
      const member2 = encoded.replaceAll("~1", "/").replaceAll("~0", "~");
      malformed3(collector, at, `${member2} accepts only: ${values.join(", ")}`);
    }
  };
...
var cost = (value, at, collector) => {
  if (isSpineRecord(value) && value["coverage"] === "unreported") {
    ...
    checkRunClosed(value, at, [...COST_MEMBERS2, { name: "coverage", check: oneOf2(["complete", "partial"]), required: false }], collector);
  }
};
...
var preparation = (value, at, collector) => {
  checkRunClosed(value, at, [
    { name: "checks", check: oneOf2(["executed", "reused"]) },
    { name: "reason", check: oneOf2(isSpineRecord(value) && value["checks"] === "reused" ? ["validation-equivalent"] : ["ordinary", "receipt-not-reusable", "preparation-fingerprint-changed"]) }
  ], collector);
};
...
var V2_PAYLOAD_MEMBERS = {
  ...PAYLOAD_MEMBERS2,
  "command.completed": [...PAYLOAD_MEMBERS2["command.completed"], { name: "preparation", check: preparation, required: false }],
...
var RUN_EVENT_PAYLOAD_GRAMMAR_SPEC = "run-event-payload-grammar/1";
function describeRunEventPayload(kind, version) {
  if (!isRunEventKind(kind)) return void 0;
  if (version === RUN_EVENT_SPEC && !RUN_EVENT_KINDS_V1.includes(kind)) return void 0;
  const rules = version === RUN_EVENT_SPEC_V2 ? V2_PAYLOAD_MEMBERS[kind] : PAYLOAD_MEMBERS2[kind];
  return {
    ...
    members: rules.map((rule) => {
      ...
      return { name: rule.name, required: rule.required !== false, ...values === void 0 ? {} : { values: [...values] } };
    })
  };
}
...
function validateRunEvent(value, options = {}) {
  ...
  const payload = value["payload"];
  checkRunClosed(payload, "/payload", v2 ? V2_PAYLOAD_MEMBERS[kind] : PAYLOAD_MEMBERS2[kind], collector);
  if (v2 && isSpineRecord(payload)) {
    if (kind === "command.completed" && payload["preparation"] !== void 0 && (payload["command"] !== "prepare" || payload["outcome"] !== "ok")) {
...
function roundKey(event) {
  ...
  return event.version === "run-event/2" ? JSON.stringify([payload["roundId"], payload["round"]]) : payload["round"];
}
function pairRounds(events) {
  ...
  const rounds = new Set([...opened, ...closed2].map((entry3) => roundKey(entry3.event)));
  const paired = [];
  let inverted = false;
  for (const round of rounds) {
    const firstOpened = first(opened.filter((entry3) => roundKey(entry3.event) === round));
    const firstClosed = first(closed2.filter((entry3) => roundKey(entry3.event) === round));
...
function governingRound(events, pairing = pairRounds(events)) {
  const latestOpened = last(indexBy(events, "review.round.opened"));
  const latestClosed = last(indexBy(events, "review.round.closed"));
  return latestOpened === void 0 || latestClosed === void 0 ? void 0 : pairing.paired.find((entry3) => entry3.openedAt === latestOpened.at && entry3.closedAt === latestClosed.at);
}
function runJournalCarries(events, entry3) {
  switch (entry3) {
    ...
    case REQUIRED.roundClosed:
      return governingRound(events) !== void 0;
    default:
...
function evaluateRunJournal(events, treeSha2, mandatedLensIds, reviewedTreeShas = []) {
  ...
  const linkedRetry = runStarted?.event.version === "run-event/2" && typeof payloadOf(runStarted.event)["predecessorRunId"] === "string";
  ...
  const requiredRound = currentRound !== void 0 && (treeSha2 === void 0 || acceptedTrees.has(String(payloadOf(currentRound.closed)["candidateTreeSha"]))) ? currentRound : void 0;
  ...
  if (gateCompletion !== void 0) {
    const closedFirst = requiredRound !== void 0 && requiredRound.closedAt < gateCompletion.at;
  ...
  if (!linkedRetry && openingGateCompletion !== void 0 && prOpened !== void 0 && prOpened.at < openingGateCompletion.at) {
  ...
  if (executorOnly && gateReported !== void 0) {
    const closedFirst = requiredRound !== void 0 && requiredRound.closedAt < gateReported.at;
    if (!closedFirst) violations.push(VIOLATION.gateReportedBeforeClosedRound);
    if (!linkedRetry && openingGateReported !== void 0 && prOpened !== void 0 && prOpened.at < openingGateReported.at) {
...
function entryHref(entryPath) {
  return pathToFileURL(entryPath).href;
}
function canonicalEntryPath(entryPath) {
  try {
    return realpathSync2(entryPath);
  } catch {
    return entryPath;
  }
}
...
async function capturePortableVerificationInputs(rootDir, config, candidate2, record2, run = runGitCommand) {
  ...
  return {
    ...
    checkBindings,
    projection,
    ...compiledPolicy === null ? {} : { compiledPolicy },
    ...waiverCandidateMatches === void 0 ? {} : { waiverCandidateMatches }
  };
}
