...
caller also supplies the round bound it declared for the delivery, the same
bound `$review-work` reduces against; the bound is not part of the harness's
comparison context and is never invented for an emission.
...
verification round, including a round
...
it was selected, contiguous over every round of the delivery before this one,
binding tuple and result together where the state is `obtained` and binding
tuple with its result-less state where the acquisition failed, so every prior
report arrives bound to its round and its `candidateRef`; plus, for each of the two `candidateRef` values
...
before; where it is unchanged the replay reopens the round the previous
candidate was reviewed under, whether that round is still open or already
closed, carrying its number, its declared lens set, and its carried findings.
...
authorized repair, or to retry an acquisition that failed, on an unchanged
candidate spends the bound like any other. The bound measures dissent,
...
removed, or renamed. The executor computes that comparison and retains its
output as evidence; a comparison that was not run, or that reports any
difference at all, is a differing identity, so a rebase whose conflict
resolution altered one delivered byte spends a round like any other change.
...
the reduction, so the entries a later round carries stay one per round, the
...
are `Binding tuple`, `Round and bound`, `Fewest-rounds mandate`, `Carry-forward
entries`, `Candidate reference resolutions`, `Charter`, `Result shape`, and
`Observation binding and selected output`. This
...
The brief names the round number and the declared bound as round k of N, and
says whether this round is the grace round obtained at the bound. Where the
round belongs to a loop that declares no bound of its own, the brief names the
round number and says the loop is unbounded. The review of a plan, which
`$plan-work` leaves boundless, is such a loop: its rounds are realized outside
...
can raise now is raised now. Beside the round and the bound, the mandate asks
the lens for a conscious effort to find everything it can within the
allocation, not across it.
...
candidate, and it never changes what the round reduces to. Such a finding
falls outside that round's delta, so the out-of-delta rule above still decides
where the lens puts it: `lateFindings` names only findings the result filed,
and a late finding that rule keeps out of `findings` is marked late in the
deferral it records instead.

The last round is judged exactly as any other, and the brief says so. A lens
neither aligns because no repair room remains nor blocks to force an
escalation; the grace round and the deferral rule handle a real finding raised
at the bound.
